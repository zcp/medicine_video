...
#### **2. 核心上下文 (Core Context)**
  ...
  * **内容来源 (Design Doc)**: `["docs/03_系统设计/直播核心功能设计文档_v6_健康检查模块设计文档.md"]`
...
当你生成新的 SQLAlchemy 模型代码时，**必须同时**更新数据库初始化脚本，否则新表不会被创建。

**注意**：健康检查模块通常不需要数据库表，但如果你需要创建其他模型，请遵循以下规范。
...
# 在 app/scripts/create_tables.py 的 init_db() 函数中添加导入
        # 导入模块模型（新增）
        from ..models.模块名 import 模型类1, 模型类2
```
...
1. **模型类清单**：列出所有新生成的模型类名称（如果适用）
2. **更新指令**：明确指出需要更新哪个初始化脚本文件（如果适用）
3. **导入语句模板**：提供准确的导入语句代码片段（如果适用）
...
1.  `直播核心功能设计文档_v6_健康检查模块设计文档---CRUD 层函数代码生成提示词文档`
2.  `直播核心功能设计文档_v6_健康检查模块设计文档---Service 层和 API 层函数代码生成提示词文档`