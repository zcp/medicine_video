...
**版本**: V2.2 (前端适配扩展版 + 图片上传功能)  
**修订日期**: 2026-01-22  
**状态**: 设计扩展完成（已包含品牌Logo上传、专家头像上传、品牌关联直播间查询接口）  
...
**P0级严重问题（7项）**：
- ✅ 新增 `session_tags` 关联表
- ✅ 新增 `live_session_experts` 中间表（场次-专家多对多关系）
...
2. 数据库Schema设计（DDL）
   ...
   - live_sessions字段扩展（summary）
   - live_session_experts（场次-专家中间表）
...
1. **基础数据表**（tags, categories, brands, experts, featured_content）：
   ...
   - 原因：可能被历史数据引用
   - 硬删除：需要 `ADMIN` 或 `SUPERADMIN` 权限 + 引用检查

2. **关联表**（session_tags, brand_topics, brand_rooms, user_favorites, user_subscriptions）：
   - 用户相关：软删除（保留审计）
   - 纯关联：硬删除（ON DELETE CASCADE）
   - **品牌直播间关联表（brand_rooms）**：硬删除（直接删除关联记录），原因：纯关联表，不需要保留历史审计记录
...
### 2.12 live_session_experts（场次专家关联表）
...
class HomepageHostInfo(BaseModel):
    ...
    title: Optional[str] = None
    hospital: Optional[str] = None
    role: Optional[str] = None  # 专家角色（主讲、主持、嘉宾）
    sort_order: Optional[int] = None  # 显示顺序
...
**请求参数 (Query)**:
- `hard_delete` (bool, 可选, 默认false): 是否硬删除（需`ADMIN` 或 SUPERADMIN）
...
2. 验证JWT Token并提取用户角色，软删除需要 `ADMIN` 或 `SUPERADMIN` 权限，硬删除需要 `ADMIN` 或 `SUPERADMIN` 权限，权限不足时返回 `3002` 错误
...
2. 检查权限：软删除需ADMIN或SUPERADMIN，硬删除需`ADMIN` 或 SUPERADMIN
...
1. 验证JWT Token并检查权限（硬删除需`ADMIN` 或 SUPERADMIN）
2. 查询品牌对象，若不存在返回 `2001`
3. 检查引用关系：
...
### 4.5 Brand_Rooms 模块API（品牌直播间关联管理）【新增】
...
3. 查询关联直播场次：
   - 查询 `live_session_experts lse JOIN live_sessions ls ON lse.session_id = ls.id JOIN live_rooms lr ON ls.room_id = lr.id`
   - 条件：`lse.expert_id = :expert_id`
   - 使用索引：`idx_live_session_experts_expert`
4. 使用 `joinedload` 预加载关联数据（session, room, statistics），避免N+1查询
...
1. 验证JWT Token并检查管理员权限
2. 根据 `expert_id` 查询专家对象，若不存在返回 `2001`
3. 由于外键设置为 `ON DELETE CASCADE`，删除专家时：
...
2. 构建查询：
   ...
   ```
3. 使用索引优化：`idx_live_session_experts_session`、`idx_live_session_experts_role`
4. 执行查询获取专家列表
5. 格式化响应数据：拼接头像URL（如果需要）
6. 返回专家列表

---

### 4.7 Session_Tags 模块API（直播场次-标签关联管理）

#### 4.7.1 为直播场次批量设置标签（Admin）
...
### 4.8 Featured Content 模块API（首页焦点图）
...
1. **`GET /api/v1/sessions/{session_id}` - 获取场次详情**
   ...
   **扩展要点**：
   - 通过 `live_session_experts` 中间表 JOIN `experts` 表获取专家列表
   - 支持一个场次有多个专家（主讲、主持、嘉宾）
   - 通过 `session_tags` JOIN `tags` 表获取标签列表
   - 使用 `joinedload` 预加载关联数据，避免N+1查询
   - 在Pydantic Schema中添加嵌套的 `SessionExpertItem` 和 `TagItem` 模型

**SessionExpertItem Schema 定义**：
```python
...
2. **`GET /api/v1/rooms/{room_id}/sessions` - 获取房间场次列表**
   ...
   **新增返回字段**（每个item）：
   - `summary`: 场次摘要
   - `experts`: 场次专家列表（通过 `live_session_experts` 中间表查询）
   - `tags`: 标签名称列表（字符串数组）
...
3. **`POST /api/v1/rooms/{room_id}/sessions` - 创建场次**
   ...
   **新增请求字段**：
   ```json
   {
     ...
     "session_experts": [
       {
         "expert_id": "expert_uuid_1",
         "role": "主讲",
         "sort_order": 0
       },
       {
         "expert_id": "expert_uuid_2",
         "role": "主持",
         "sort_order": 1
       }
     ]
   ...
   **扩展要点**：
   - 在创建场次时支持设置 `summary` 和 `session_experts`（专家列表）
   - 验证所有 `expert_id` 在 `experts` 表中存在
...
4. **`PATCH /api/v1/sessions/{session_id}` - 更新场次**
   ...
   **新增可更新字段**：
   - `summary`: 更新场次摘要
   - `session_experts`: 更新专家列表（全量替换）
   
   **扩展要点**：
   - 更新时验证新的 `expert_id` 有效性
   - 记录INFO日志包含更新的字段
...
2. 构建复杂的联表查询（核心逻辑）：
   ...
   - 使用子查询或窗口函数找到每个房间的"相关场次"（relevant_session）
     ...
     - `SELECT ls.id, ls.status, ls.start_time, ls.end_time FROM live_sessions ls WHERE ls.room_id = lr.id ORDER BY (优先级逻辑) LIMIT 1`
3. LEFT JOIN `live_session_experts lse` 获取场次关联的专家信息
   - 查询条件：`session_id = ls.id AND role = '主讲'`
   - 按 `sort_order` 排序，取第一个
   - 如果没有主讲专家，降级查找其他角色（主持、嘉宾）
4. LEFT JOIN `experts e` ON `lse.expert_id = e.id` 获取专家的详细信息（name、title、hospital等）
5. LEFT JOIN `session_statistics ss` 获取统计数据：`ON ls.id = ss.session_id`
6. 在Service层应用Host选择逻辑（从 live_session_experts 获取专家信息）
   - 如果找到主讲专家，使用主讲专家信息
   - 如果没有主讲专家，使用其他角色专家信息（主持、嘉宾）
   - 如果场次没有关联任何专家，`host` 为 `null`
7. 根据 `relevant_session` 的status确定 `live_status`（映射关系：live→live, scheduled→scheduled, ready/ended→replay）
8. 计算热度（heat）：使用 `session_statistics` 数据和预定义公式
9. 构造 `status_data`：根据 `live_status` 填充相应字段（live填充viewer_count, scheduled填充start_time, replay填充duration和play_count）
10. 执行COUNT查询获取筛选后的总房间数
11. 解析 `sort` 参数并应用排序（热度、开始时间、创建时间等）
12. 应用分页（`LIMIT :size OFFSET :offset`）
13. 执行主查询获取当前页的房间列表
14. 将结果映射到 `HomepageRoomItem` Schema
15. 构建分页响应并返回
...
| 资源不存在 | 2001 | 资源不存在 | 所有GET/PATCH/DELETE接口（包括品牌、专题、直播间不存在） |
| 资源已存在（名称冲突） | 2002 | 资源已存在 | 所有POST接口（name唯一性检查） |
| 操作被禁止（被引用） | 2003 | 操作被禁止 | 所有DELETE接口（引用检查，包括品牌被专题或直播间引用） |
| 业务逻辑错误（user_id已绑定） | 2004 | 业务逻辑错误 | POST/PATCH experts（user_id冲突） |
| 未认证（Token缺失/无效） | 3001 | 未认证 | 所有需认证的接口 |
| 权限不足 | 3002 | 权限不足 | 所有Admin接口（角色检查） |
| 参数校验失败 | 4001 | 参数校验失败 | 所有接口（Pydantic验证失败、专题ID无效、品牌ID无效、直播间ID无效） |
...
2. **关联表创建**: session_tags, brand_topics, brand_rooms【新增】
3. **字段补充**: 
...
2. **关联管理接口**: 品牌-专题、品牌-直播间【新增】、场次-标签的批量操作
3. **搜索功能**: 按标签搜索直播
