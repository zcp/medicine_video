...
## 十七、管理端全站房间列表 + 私密测播（文档 17 / 18）

> **专篇**：[`17-18-管理端与私密测播-前端对接文档.md`](./17-18-管理端与私密测播-前端对接文档.md)  
> **后端状态**：✅ 已落地（`live_core_service`）  
> **前端仓库**：`Live-Saas-Wechat`

### 17.1 后端 API 清单

| # | 方法 | 路径 | 权限 | 用途 |
|---|------|------|------|------|
| 1 | GET | `/admin/rooms` | Admin | 全站房间列表（含不公开/测播；无 stream_key） |
| 2 | GET/PATCH/DELETE | `/rooms/{id}` | Admin\|owner（读：持链均可） | 详情/改/删 |
| 3 | POST | `/rooms/{id}/cover` | Admin\|owner | 封面 |
| 4 | POST | `/rooms/{id}/test-room` | owner\|Admin | **新增** 确保测播间（幂等） |
| 5 | GET | `/homepage/rooms` | 公开 | 已排除 `is_private=true` |
| 6 | GET | `/search` | 公开 | room 结果已排除不公开 |

### 17.2 关键语义（前端必读）

- `is_private=true` = **不公开（unlisted）**：不进广场；**有 roomId 可看，无密码**。
- UI 文案用「不公开」，避免只写「私密」。
- Home 三 Tab **不要**再做私密过滤；以后端为准。
- 「我的直播」`/users/me/rooms` 仍含自己的测播间。

### 17.3 管理端页面（已有骨架）

| 页面 | 路径建议 | 说明 |
|------|----------|------|
| 全站房间列表 | `pages/admin/room/AdminRoomList` | 已调 `ADMIN.ROOMS`；文案已改「不公开」 |
| 编辑弹层 | `AdminRoomEditDialog` | 复用 PATCH/cover；提示已对齐 unlisted |
...
LiveView：若 `is_private===true`，顶栏弱提示「连接测试」。
...
interface TestRoomEnsureResponse {
