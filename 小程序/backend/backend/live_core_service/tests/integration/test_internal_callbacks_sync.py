...
async def test_on_publish_for_scheduled_session(
    ...
    async for client in async_client:
        async for db in db_session:
            ...
            response = await client.post("/api/v1/internal/srs/on_publish", json=payload)
...
async def test_on_unpublish_and_task_success(
    ...
    """
    测试 on_unpublish 回调及后续 Celery 任务成功执行

    流程:
    1. 创建测试房间和 live 状态的会话
    2. 调用 on_unpublish 端点
    3. 验证会话最终状态为 ready（经历完整的状态转换）
    """
    async for client in async_client:
        async for db in db_session:
...
async def test_on_unpublish_and_task_failure(
    ...
    """
    测试 on_unpublish 回调及后续 Celery 任务失败的情况
    ...
    async for client in async_client:
        async for db in db_session:
            ...
            # Arrange: 模拟任务处理过程中的失败
            mock_sleep = mocker.patch("app.tasks.session_processing.time.sleep")
            mock_sleep.side_effect = Exception("模拟的视频处理失败")
