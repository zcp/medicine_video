...
@pytest.mark.asyncio
async def test_get_user_notification_detail_success(async_client, db_session, regular_user_token):
    """获取通知详情成功"""
    async for db in db_session:
        payload = jwt.decode(regular_user_token, settings.JWT_SECRET_KEY, algorithms=["HS256"])
        user_id_from_token = uuid.UUID(payload.get("user_id"))
        ...
        async for client in async_client:
            ...
            assert data["data"]["title"] == "详情通知"
            assert data["data"]["content"] == "这是一条用于详情页测试的通知内容。"
...
@pytest.mark.asyncio
async def test_get_user_notification_detail_not_found(async_client, regular_user_token):
    """获取不存在的通知详情时返回404"""
    async for client in async_client:
        headers = {"Authorization": f"Bearer {regular_user_token}"}
        resp = await client.get(
            f"/api/v1/users/me/notifications/{uuid.uuid4()}",
            headers=headers,
        )
