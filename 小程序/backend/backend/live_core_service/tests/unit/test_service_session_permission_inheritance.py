...
class TestSessionPermissionInheritance:
    ...
    @pytest.mark.asyncio
    async def test_session_inherits_private_room_invisibility_for_anonymous(
        self,
        db_session,
        regular_user_id: uuid.UUID
    ):
        """测试：场次继承Private房间的不可见性（匿名用户）"""
        async for db in db_session:
            # ===== Arrange (准备) =====
            # 创建Private房间
            from app.crud import room as crud_room
            ...
            # ===== Act & Assert (执行 & 断言) =====
            # 匿名用户访问该房间的Session列表应抛出404
            with pytest.raises(NotFoundException):
                await session_service.get_sessions_by_room(
                    room_id=room.id,
                    page=1,
                    size=10,
                    user_id=None,  # 匿名用户
