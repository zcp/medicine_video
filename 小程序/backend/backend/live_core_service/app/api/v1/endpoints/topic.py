...
router = APIRouter(tags=["Topics"])  # prefix 在 api.py 中指定
...
@router.post("/{topic_id}/categories")
...
@router.get("/{topic_id}/categories")
...
async def get_rooms_in_category(
    ...
    except (CategoryNotFoundException, TopicNotFoundException, NotFoundException) as e:  # ← 新增：捕获NotFoundException（404伪装）
        # ← 新增：返回404（隐藏Draft/Archived专题存在性）
