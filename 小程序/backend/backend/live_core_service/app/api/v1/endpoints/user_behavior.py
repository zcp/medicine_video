...
@router.post("/users/me/watch-history")
...
@router.get("/users/me/watch-history")
...
@router.delete("/users/me/watch-history/{history_id}")
...
@router.post("/users/me/subscriptions")
...
@router.delete("/users/me/subscriptions")
async def cancel_subscription(
    ...
    try:
        ...
        logger.warning(f"取消订阅失败（资源不存在）: user_id={user_id_for_logging}, target_id={str(target_id)[:8]}, error={str(e)}")
        return JSONResponse(
            status_code=404,
            content=error_response(code=2001, message="资源不存在")
        )
