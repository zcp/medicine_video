...
async def create_favorite(
    ...
    try:
        ...
        logger.info(
            "[CRUD] 创建收藏成功: user_id=%s, room_id=%s, favorite_id=%s, obj_type=%s",
            str(user_id)[:8],
            str(room_id)[:8],
            str(new_fav.id)[:8],
            type(new_fav).__name__,
        )
        return new_fav
...
async def get_user_favorites(
    ...
    """获取指定用户的收藏列表（分页），仅返回 is_active=True 的记录，按创建时间倒序。"""
    stmt = (
        select(UserFavorite)
        .where(
            UserFavorite.user_id == user_id,
...
async def record_watch_history(
    ...
    try:
        ...
        db.add(history)
        await db.flush()
        await db.refresh(history)  # 刷新以获取数据库端生成的值（如 watched_at），避免后续 model_validate 触发 MissingGreenlet
        logger.info(
...
async def count_watch_history(db: AsyncSession, user_id: UUID) -> int:
    """获取用户观看历史总数（is_latest=True）。"""
    stmt = select(func.count()).select_from(WatchHistory).where(
        WatchHistory.user_id == user_id,
        WatchHistory.is_latest.is_(True),
    )
    result = await db.execute(stmt)
...
async def list_watch_history(
    ...
    """获取用户观看历史（分页，只返回 is_latest=True 的记录），按 watched_at 降序。"""
    stmt = (
        select(WatchHistory)
        .where(
            WatchHistory.user_id == user_id,
...
async def create_subscription(
    ...
    if sub:
        ...
        sub.is_active = True
        await db.flush()
        await db.refresh(sub)  # 刷新以获取数据库端生成的值（如 created_at），避免后续 model_validate 触发 MissingGreenlet
        logger.info(
    ...
    try:
        await db.flush()
        await db.refresh(new_sub)  # 刷新以获取数据库端生成的值（如 created_at），避免后续 model_validate 触发 MissingGreenlet
        logger.info(
...
async def list_subscriptions(
    ...
    """获取用户的订阅列表（分页），可按 target_type 过滤。"""
