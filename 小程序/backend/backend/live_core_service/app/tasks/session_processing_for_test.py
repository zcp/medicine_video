...
# 5. (核心修改) 将 Celery 入口任务改为异步，并负责管理会话生命周期
async  def post_stream_processing_task(self, session_id: str):
    """
    Celery 的原生异步入口任务。
    它的职责是创建并管理数据库会话，然后调用核心业务逻辑。
    """
    logger.info(f"[CELERY_TASK] 收到任务，准备执行: session_id={session_id}")

    # 从依赖生成器中获取会话
    async_db_gen = get_async_db()
    #db: AsyncSession = await anext(async_db_gen)
