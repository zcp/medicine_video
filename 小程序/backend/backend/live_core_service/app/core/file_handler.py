...
UPLOAD_ALLOWED_EXTENSIONS = os.getenv("UPLOAD_ALLOWED_EXTENSIONS", "jpg,jpeg,png,webp").split(",")

# 允许的MIME类型（焦点图仅支持 jpg/png/webp）
...
class FileHandler:
    ...
    def generate_featured_content_image_path(content_id: uuid.UUID, extension: str) -> Tuple[str, str]:
        ...
        Args:
            content_id: 焦点图ID（仅用于日志，文件名使用独立UUID）
            extension: 文件扩展名
