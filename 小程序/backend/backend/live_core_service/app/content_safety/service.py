    ...
    *,
    ...
    for item in items:
        ...
        if decision in ("warn", "block") or matched_rules or political_hit:
            ...
            try:
                ...
                # 审计失败不得盖住业务拦截决策（否则前端只看到 500）
                logger.warning(
                    "内容安全审计日志写入失败（已忽略）: scene=%s field=%s error=%s",
                    scene,
                    item.field_name,
                    log_exc,
                    exc_info=True,
    ...
    if overall_decision == "block":
        ...
        raise ContentSafetyBlockedException(
            block_messages[0] if block_messages else "内容未通过审核，请修改后重试",
        )
...
def assert_content_safe_or_raise(result: ContentSafetyCheckResult) -> None:
    if not result.passed:
        raise ContentSafetyBlockedException("内容未通过审核，请修改后重试")
