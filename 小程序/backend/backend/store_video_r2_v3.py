...
def upload_hls_to_r2_no_chunk(local_hls_folder):
    r2_client, bucket_name = create_r2_client()
    try:
        folder_name = os.path.basename(os.path.normpath(local_hls_folder))

        for root, _, files in os.walk(local_hls_folder):
            for file in files:
                local_file_path = os.path.join(root, file)
                relative_path = get_relative_path(local_file_path, local_hls_folder)
                object_name = f"videos/hls/{folder_name}/{relative_path}"
        ...
        # 返回对应目录的播放列表地址
        playlist_url = construct_subdomain_r2_url(f"videos/hls/{folder_name}/playlist.m3u8")
        logging.info(f"✅ HLS 主播放列表地址: {playlist_url}")
        return playlist_url
    except Exception as e:
        logging.error(f"HLS 上传失败: {e}")
...
def test():
    ...
    hls_folder = "D:\\vzan\\downloaded_media\\84105484_cf73"
    upload_hls_to_r2_no_chunk(hls_folder)

    hls_folder = "D:\\vzan\\downloaded_media\\216437500_b3e3"
    upload_hls_to_r2_no_chunk(hls_folder)
