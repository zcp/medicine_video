...
from app.services.tencent_sms_provider import TencentCloudSmsProvider
...
from app.services.carrier_auth_provider import (
    AliyunCarrierAuthProvider,
    DCloudCarrierAuthProvider,
...
TERMS_NOT_AGREED_MESSAGE = "请先同意服务条款和隐私政策"


def require_agreed_to_terms(agreed: bool) -> None:
    """登录/注册前的服务条款同意校验（与 V3/V4 一键、微信登录一致）。"""
    if not agreed:
        logger.warning("认证失败：用户未同意服务条款")
        raise InvalidCredentialsException(TERMS_NOT_AGREED_MESSAGE)
...
class AuthService:
    ...
    def __init__(self, db: AsyncSession, redis_client=None):
        ...
        self.email_provider: EmailProvider = self._build_email_provider()
        self.carrier_auth_provider: CarrierAuthProvider = self._build_carrier_auth_provider()
        self.wechat_auth_provider: WeChatAuthProvider = self._build_wechat_auth_provider()
    ...
    def _build_sms_provider(self) -> SmsProvider:
        ...
        if settings.SMS_PROVIDER == "tencent":
            return TencentCloudSmsProvider(
                ...
                template_code=settings.TENCENT_SMS_TEMPLATE_CODE,
            )
        raise ValueError("SMS_PROVIDER must be mock, aliyun, or tencent")

    def _build_email_provider(self) -> EmailProvider:
        """Build email provider from explicit configuration."""
        ...
        raise ValueError("EMAIL_PROVIDER must be mock or smtp")

    def _build_carrier_auth_provider(self) -> CarrierAuthProvider:
        """Build carrier auth provider from explicit configuration."""
        if settings.CARRIER_AUTH_PROVIDER == "mock":
            return MockCarrierAuthProvider()
        if settings.CARRIER_AUTH_PROVIDER == "aliyun":
            return AliyunCarrierAuthProvider(
                access_key_id=settings.ALIYUN_ACCESS_KEY_ID,
                access_key_secret=settings.ALIYUN_ACCESS_KEY_SECRET,
            )
        if settings.CARRIER_AUTH_PROVIDER == "dcloud":
            return DCloudCarrierAuthProvider()
        raise ValueError("CARRIER_AUTH_PROVIDER must be mock, aliyun, or dcloud")

    def _build_wechat_auth_provider(self) -> WeChatAuthProvider:
        """Build WeChat mini-program auth provider from explicit configuration."""
        if settings.WECHAT_AUTH_PROVIDER == "mock":
            return MockWeChatAuthProvider()
        if settings.WECHAT_AUTH_PROVIDER == "official":
            return OfficialWeChatAuthProvider(
                appid=settings.WECHAT_MINI_APPID,
                secret=settings.WECHAT_MINI_SECRET,
            )
        raise ValueError("WECHAT_AUTH_PROVIDER must be mock or official")
    ...
    def _create_access_token(
        ...
        Args:
            ...
            user_role: 用户角色，如果为None或空字符串，则设置为空字符串""表示访客
            username: 用户名（可选，用于前端显示）
            nickname: 用户昵称（可选，用于前端显示）
            can_stream: 是否可开播；缺省 true；false 表示被禁止开播
            avatar_url: 头像 URL（可选，供留言等场景快照展示）
        """
        now = datetime.utcnow()
        ...
        # 可选：在JWT中包含用户名、昵称、头像，方便前端与留言快照展示
        if username:
    ...
    async def send_otp_code(self, otp_request: schemas.VerificationCodeRequest, client_ip: str = None) -> dict:
        ...
        try:
            ...
            # 6. 调用 provider 发送（SMS → 短信，EMAIL → 邮件）
            ...
            elif otp_request.channel == "EMAIL":
                sent = await self.email_provider.send(
                    to=otp_request.recipient,
                    code=otp_code,
                    subject="您的验证码",
                )
                if not sent:
                    raise RuntimeError("邮件服务发送失败")
    ...
    async def login_user(self, login_request: schemas.LoginRequest, client_ip: str = None) -> dict:
        ...
        require_agreed_to_terms(getattr(login_request, "agreed_to_terms", False))
        ...
        try:
            ...
            # 2. 用户查询
            user = await crud_user.get_by_login_identifier(self.db, identifier=login_request.username)
            if not user:
    ...
    async def refresh_access_token(self, refresh_token: str) -> dict:
        ...
        try:
            ...
            # 获取角色值与开播资格（必须从 DB 重载，否则授/撤权后 refresh 仍带旧声明）
            user_role = user.role.value if hasattr(user.role, 'value') else str(user.role)
            user_can_stream = bool(getattr(user, "can_stream", True))
            user_username = getattr(user, "username", None)
            user_nickname = getattr(user, "nickname", None)
            ...
            logger.info(
                f"成功刷新令牌: user_id={user_id}, role={user_role}, can_stream={user_can_stream}"
            )
    ...
    async def request_password_reset(self, reset_request: schemas.PasswordResetRequest):
        ...
        try:
            ...
            expires_seconds = 900  # 15 分钟（与 Redis TTL 一致）
            
            # 5. 存入缓存
            reset_key = f"password_reset:{reset_token}"
            await self.redis_client.set(reset_key, str(user.id), ex=expires_seconds)
            
            # 6. 发送重置邮件（含 reset_token；契约见《10》§3.1.7 / 前端指南）
            sent = await self.email_provider.send_password_reset(
                to=reset_request.email,
                reset_token=reset_token,
                expires_minutes=expires_seconds // 60,
                subject="密码重置",
            )
            if not sent:
                # 发信失败则作废 token，避免用户无邮件却留下可用重置凭据
                await self.redis_client.delete(reset_key)
                logger.error(
                    f"密码重置邮件发送失败: email={self._mask_recipient(reset_request.email)}"
                )
                raise RuntimeError("邮件服务发送失败")
            
            logger.info(
                f"成功处理密码重置请求并已发信: email={self._mask_recipient(reset_request.email)}"
            )
    ...
    async def login_by_phone_ticket(self, login_request) -> dict:
        ...
        # 先窥视 channel，避免 EMAIL ticket 被错误端点误消费
        ticket_preview = await self._get_ticket("LOGIN", login_request.login_ticket)
        if ticket_preview.get("channel") != "SMS":
    ...
    async def login_by_email_ticket(self, login_request) -> dict:
        """邮箱验证码登录（V5）：消费 LOGIN ticket（channel=EMAIL）后签发 JWT。"""
        require_agreed_to_terms(getattr(login_request, "agreed_to_terms", False))

        # 先窥视 channel，避免 SMS ticket 被错误端点误消费
        ticket_preview = await self._get_ticket("LOGIN", login_request.login_ticket)
        if ticket_preview.get("channel") != "EMAIL":
            raise InvalidCredentialsException("登录凭证无效")

        ticket_data = await self._consume_ticket("LOGIN", login_request.login_ticket)

        email = ticket_data.get("recipient")
        if not email:
            raise InvalidCredentialsException("登录凭证无效")

        user = await crud_user.get_by_email(self.db, email)
        if not user:
            raise InvalidCredentialsException("登录凭证无效")
        ...
        logger.info(f"邮箱验证码登录成功: user_id={user_id}, email={self._mask_recipient(email)}")
        ...
        return {
            ...
            "username": user_username or "",
            "nickname": user_nickname or "",
            "email_masked": self._mask_recipient(email),
        }
    ...
    async def register_by_phone_ticket(self, register_request) -> dict:
        ...
        user_can_stream = bool(getattr(new_user, "can_stream", True))
        access_token = self._create_access_token(
            user_public_id,
            "REGULAR",
            username=_reg_username,
            nickname=_reg_nickname,
            can_stream=user_can_stream,
    ...
    async def one_tap_login(self, one_tap_request) -> dict:
        ...
        require_agreed_to_terms(getattr(one_tap_request, "agreed_to_terms", False))
    ...
    async def one_tap_login_direct(
        self,
        phone: str,
        sign: str,
        timestamp: str,
        ...
        Raises:
            ...
            InvalidCredentialsException: 未同意条款或用户状态异常
        """
        require_agreed_to_terms(agreed_to_terms)
    ...
    async def wechat_login(self, wechat_request) -> dict:
        """微信小程序一键授权登录：code → openid → 查/建用户 → 签发 JWT

        Args:
            wechat_request: WeChatLoginRequest，含 code、agreed_to_terms

        Returns:
            {access_token, refresh_token, token_type, is_new_user,
             user_public_id, username, nickname, phone_masked}

        Raises:
            InvalidCredentialsException: 未同意条款或账户状态异常
            InvalidTokenException: code 无效 / 微信换号失败
        """
        require_agreed_to_terms(getattr(wechat_request, "agreed_to_terms", False))

        try:
            openid = await self.wechat_auth_provider.exchange_code(wechat_request.code)
        except (ValueError, NotImplementedError) as e:
            logger.warning(f"微信登录换号失败: {e}")
            raise InvalidTokenException("微信登录失败，请重试")

        user = await crud_user.get_by_social_id(
            self.db, provider="wechat", social_id=openid
        )
        is_new_user = False
        ...
        if not user:
            ...
            try:
                user = await crud_user.create(self.db, user_create, password_hash=None)
                is_new_user = True
                logger.info(f"微信登录自动注册: user_id={user.id}")
                ...
                if not user:
                    raise
                is_new_user = False
                logger.info(f"微信登录并发冲突后命中已有用户: user_id={user.id}")
        ...
        logger.info(f"微信登录成功: user_id={user_id}, is_new={is_new_user}")
        ...
        return {
            ...
            "username": user_username or "",
            "nickname": user_nickname or "",
            "phone_masked": self._mask_recipient(phone_number) if phone_number else None,
        }
    ...
    async def _get_ticket(self, purpose: str, ticket_id: str) -> dict:
        """读取 ticket 但不消费（用于 channel 预校验）。"""
