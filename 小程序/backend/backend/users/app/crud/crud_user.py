...
async def get_by_uuids(db: AsyncSession, public_ids: List[uuid.UUID]) -> List[User]:
    """按 public_id 批量查询用户（服务间展示字段补全用）"""
    if not public_ids:
        return []
    try:
        stmt = select(User).where(User.public_id.in_(public_ids))
        result = await db.execute(stmt)
        return list(result.scalars().all())
    except Exception as e:
        logger.error(f"批量查询用户失败: count={len(public_ids)}, error={e}")
        raise
    ...
    *, 
    ...
    try:
        ...
        if filters.can_stream is not None:
            ...
            logger.debug(f"添加开播筛选: {filters.can_stream}")
