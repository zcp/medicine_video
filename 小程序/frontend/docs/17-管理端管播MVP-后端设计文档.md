...
**日期**: 2026-07-24  
**状态**: ✅ 设计完成，可直接开发 / 以复用为主  
**基于**: 《后端设计文档模板》；《15-全站已实现功能全景地图…》；《16-D1》；《FRONTEND_IMPLEMENTATION_GUIDE》；现网 `RoomService._check_write_permission`  
...
| V1.0 | V1.1 |
|------|------|
| 以场次三态 + force-end / hide-replay 为 P0 | **砍掉**播控 P0；改为房间内容运营 |
| `GET /admin/sessions` | 改为 `GET /admin/rooms` |
| 3 个新动作 API | **仅 1 个**新列表 API；写操作全复用 |
...
**包含能力（P0）**:
- **全站房间列表（Admin Room List）**: 管理员按标题/房主找到任意直播间
- **内容编辑（Content Edit）**: 标题、简介、封面、私密、Tab、科室、品牌、公众号、留言清理、删房等——**复用现网**
...
| 2 | CRUD/Service：Admin 全站房间分页（含私密）、标题 ILIKE、按 `owner_user_id` 筛 | `crud/room.py` + `services/` | ⭐ | §3.1 |
...
|--|--------------|---------------------------|
| 权限 | Optional Auth；Admin 可见私密 | **仅** Admin |
| 列表字段 | id/title/cover/parent/created_at | **加** owner、description、is_private、updated_at 等 |
| 用途 | C 端/通用 | 运营后台 |
...
class AdminRoomQueryParams(BaseModel):
    ...
    is_private: Optional[bool] = Field(None, description="按私密筛选；省略=全部")
...
| 操作 | 方法路径 | 说明 |
|------|----------|------|
| 详情 | `GET /rooms/{room_id}` | Admin 可读私密房 |
| 改标题/简介/私密等 | `PATCH /rooms/{room_id}` | body: `LiveRoomUpdate` |
...
| 3 | ADMIN 无筛选 | 含他人房 + 私密房 | `[x]` superadmin total=9 |
...
| 步骤 | 调用 |
|------|------|
| 列表 | `GET /api/core/admin/rooms?q=&owner_user_id=&page=&size=` |
| 详情 | `GET /api/core/rooms/{id}` |
| 改文案 | `PATCH /api/core/rooms/{id}` |
