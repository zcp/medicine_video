...
> ⚠️ **已升版**：请以 [《17-…-前端设计文档-v1.1》](./Live-Saas-Wechat-17-管理端直播间内容运营MVP-前端设计文档-v1.1.md) 为准（2026-08-08：对齐《18》「不公开」管理端口径）。本文 V1.0 仅作历史参考。

**项目**: Live-Saas-Wechat  
**模块编号**: 17  
**版本**: V1.0  
**创建日期**: 2026-07-24  
**状态**: 📋 设计定稿，可直接开发（以复用为主）· **已被 V1.1 取代**  
**技术栈**: uni-app + Vue 3 + TypeScript + Pinia  
**平台**: 微信小程序

> **后端设计文档**：《17-管理端管播MVP-后端设计文档.md》V1.1  
...
|------|--------|----------------|----------|------|
| **找任意直播间** | ADMIN / SUPERADMIN | 个人中心 → **管理功能 → 直播间运营** | 标题搜索 / 房主 UUID / 私密筛选 | 走新接口 `GET /admin/rooms` |
| **改标题 / 简介 / 私密** | 同上 | 列表 → **编辑内容** | 弹窗保存 → `PATCH /rooms/{id}` | Admin 可改**他人**房 |
...
**包含能力（P0）**:
- **全站房间列表**：标题模糊搜、按房主筛、含私密房、展示 `owner_user_id`
- **内容编辑**：标题 / 简介 / 私密 / 封面 —— 复用现网房间写接口
- **子资源快捷入口**：Tab / 科室 / 品牌 / 留言 / 删房 —— 复用既有页面与 API
- **权限 UI**：仅 `isAdmin` 可见入口；列表遇 `3003` 友好提示

**不包含（P0）**:
- 强制结束直播、暂停、踢流、下架回放、取消预告专用包
- 新造 `PATCH /admin/rooms/{id}` 或第二套写路径
- 列表展示 `stream_key`（后端亦不返回）
- 场次三态列表 / 播控台
...
1. **直播间运营列表页**：分页、标题 `q`、房主 `owner_user_id`、私密筛选；加载 / 空 / 错误态  
2. **内容编辑弹窗**：标题、简介、是否私密、换封面；保存走 `updateRoom` / `uploadRoomCover`  
...
│   │       └── AdminRoomEditDialog.vue      # 【新增】标题/简介/私密/封面编辑弹窗
│   ├── profile/
...
| 改标题/简介/私密等 | `updateRoom(id, data)` | `PATCH /rooms/{id}` |
...
- `is_private` 仅在用户选择「仅私密 / 仅公开」时传 `true`/`false`；选「全部」时省略  
...
**职责（P0 最小）**: 改标题、简介、私密、封面；提供子模块入口与删房。
...
| 2 | GET | `/api/v1/rooms/{id}` | `/api/core/rooms/{id}` | JWT（Admin 可读私密） | `getRoomById` | 复用 |
...
5. **文案**: 菜单用「直播间运营」；勿用「管播台 / 强制下播」等易误解为播控的 P0 文案  
