...
### 7.1. 设计原则
  ...
  * **数据完整性**: 使用 `FOREIGN KEY`, `UNIQUE`, `ENUM` 保证数据准确。
...
CREATE TABLE live_rooms (
 -- 核心标识 (在应用层通过 uuid.uuid4() 生成)
    ...
    -- 所有权
    user_id UUID NULL, -- [待办] 初期允许为空。未来在用户模块实现后，应变更为 NOT NULL 并添加 FOREIGN KEY REFERENCES users(id)。
...
2.  **阶段二 (房间管理)**: 实现 `/api/v1/rooms` 的完整 CRUD 接口。
3.  **阶段三 (直播集成)**: 配置 SRS，实现 `/internal/srs` 的两个回调接口，完成直播生命周期的闭环。
4.  **阶段四 (统计与查询)**: 实现后台定时任务，并实现 `/api/v1/sessions` 的查询接口。
5.  **阶段五 (前端对接与部署)**: 开发简易前端播放页面，配置 Nginx，完成部署。
