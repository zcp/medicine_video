...
##### **1.1. 上传/更新直播间封面 (新增)**
  ...
  * **失败响应示例** (`400 Bad Request` - 文件格式或大小不符):
    ...
    ```
  * **实现流程描述**:
    1.  验证JWT Token，提取 `user_id`。
    2.  定义 FastAPI 路由函数，接收路径参数 `room_id: UUID` 和文件对象 `file: UploadFile`。
    3.  查询数据库，确认 `room_id` 对应的房间存在且 `user_id` 匹配当前用户。若不匹配，返回 403 Forbidden。
...
##### **2. 获取单场直播的详细信息**
  ...
  * **实现流程描述 (新增)**:
    ...
    4.  **构建响应**:
          ...
          * **生成回放地址 (核心逻辑)**:
              * 检查 `session` 对象的 `status` 字段。
              * **如果 `status` 为 `'ready'` 且 `video_id` 存在**，则根据“7.1. 回放URL生成规范”动态构建完整的 `playback_url`。
              * **如果 `status` 不是 `'ready'`** (例如 `'live'`, `'processing'`, `'error'`)，则 `playback_url` 字段应返回 `null`。
    5.  构建并返回最终的成功响应。
-----
...
所有视频回放地址 (`playback_url`) 将由后端动态生成，并遵循以下统一格式：

**URL 格式**:
`<Base_URL>` + `<Storage_Path>`

  * **`<Base_URL>`**: 系统的媒体资源访问基地址，应配置为环境变量（例如 `https://your-cdn.com` 或 `https://your-domain.com`），指向Nginx或CDN。
  * **`<Storage_Path>`**: 指向经过转码处理后，最适合回放的MP4文件。根据 **7.1.1. 存储路径规范**，其格式为：
    `/media/video_{video_id}/mp4/video_{video_id}_transcoded_{quality}_{timestamp}.mp4`

**生成条件**:
只有当 `live_sessions` 表中的记录满足以下两个条件时，后端才会生成 `playback_url`：
