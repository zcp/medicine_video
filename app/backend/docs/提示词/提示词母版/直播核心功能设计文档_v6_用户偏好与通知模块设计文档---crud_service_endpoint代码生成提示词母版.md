...
#### **2. 核心上下文 (Core Context)**
  ...
  * **内容来源 (Design Doc)**: `["docs/03_系统设计/直播核心功能设计文档_v6_用户偏好与通知模块设计文档.md"]`
...
当你生成新的 SQLAlchemy 模型代码（如 `app/models/user_preference.py`）时，**必须同时**更新数据库初始化脚本，否则新表不会被创建。
...
# 用户偏好与通知模块模型（新增）
from .user_preference import UserPreference, Notification
```

**操作指令**（最小幅度修改要求）：
- **必须**识别新生成的所有模型类名称（UserPreference, Notification）
...
# 在 app/scripts/create_tables.py 的 init_db() 函数中添加导入
        # 导入用户偏好与通知模块模型（新增）
        from ..models.user_preference import UserPreference, Notification
```
...
1. **模型类清单**：列出所有新生成的模型类名称（UserPreference, Notification）
...
1.  `直播核心功能设计文档_v6_用户偏好与通知模块--CRUD 层函数代码生成提示词文档`
2.  `直播核心功能设计文档_v6_用户偏好与通知模块--Service 层和 API 层函数代码生成提示词文档`