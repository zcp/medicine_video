...
#### **2. 核心上下文 (Core Context)**
  ...
  * **内容来源 (Design Doc)**: `["docs/03_系统设计/直播核心功能设计文档_v6_首页与搜索模块设计文档-焦点图-首页API-搜索API.md"]`
...
当你生成新的 SQLAlchemy 模型代码（如 `app/models/featured_content.py`）时，**必须同时**更新数据库初始化脚本，否则新表不会被创建。
...
**操作指令**（最小幅度修改要求）：
- **必须**识别新生成的所有模型类名称（FeaturedContent等）
...
# 在 app/scripts/create_tables.py 的 init_db() 函数中添加导入
        # 导入首页与搜索模块模型（新增）
        from ..models.featured_content import FeaturedContent
```
