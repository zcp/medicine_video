...
> 实际代码实现与原 P2 设计存在以下偏差，均已在实际代码中落地。

### 8.1 函数清单差异

| 原设计（7 个函数） | 实际实现（10 个函数） | 说明 |
|---|---|---|
| `get_departments_paginated` | ✅ 已实现 | 增强：增加了 `joinedload(ExpertDepartment.category)` 预加载 + 批量 `expert_count` 计算 |
| `get_department_by_id` | ✅ 已实现 | 一致 |
| `get_department_by_name` | ✅ 已实现 | 一致 |
| `create_department` | ✅ 已实现 | 一致 |
| `update_department` | ✅ 已实现 | 差异：返回 `Optional[ExpertDepartment]`（不存在时返回 None），而非设计中的抛异常 |
| `soft_delete_department` | ✅ 已实现 | 差异：返回 `bool`（不存在时返回 False），而非设计中的抛异常 |
| `get_unmapped_experts` | ✅ 已实现 | 一致 |
| — | **新增** `transfer_experts_to_department` | 支撑 merge 操作：批量将源科室的专家转移到目标科室 |
| — | **新增** `hard_delete_department` | 支撑 merge 操作：物理删除被合并的源科室 |
| — | **新增** `update_experts_category_by_department` | 支撑 update_category 操作：应用层批量同步专家 category_id |

### 8.2 `create_expert_raw` 未实现

原设计 §5.2 改动 4 要求新增 `create_expert_raw(db, data: dict)` 函数，接收裸 dict。

**实际实现**：未新增此函数。Service 层直接复用已有的 `create_expert(db, ExpertCreate)`，在调用前通过修改 Pydantic 对象的 `department_id`/`category_id` 字段来传递解析结果。

**判断**：**实现更优**。原有 `create_expert` 的 `try/except IntegrityError` 模式和所有日志/错误处理逻辑可以完全复用，新增一个 dict 版本会增加维护负担而无实际收益。

### 8.3 keyword 搜索子查询 — 已按设计实现

原设计 §4.3 要求将 `Expert.department.ilike(kw)` 改为子查询 `Expert.department_id.in_(select(ExpertDepartment.id)...)` — **✅ 已正确实现**。

`department` 筛选参数的模糊匹配也同步改为子查询（§4.4）。

### 8.4 返回值模式差异

原设计要求 CRUD 函数在记录不存在时抛异常。实际实现遵循了项目既有的 `update_expert`/`delete_expert` 模式：CRUD 层返回 `None`/`False` 表示不存在，由 Service 层决定是否抛 404。

**判断**：**实现更优**。CRUD 层不抛业务异常是 Clean Architecture 的约定。
