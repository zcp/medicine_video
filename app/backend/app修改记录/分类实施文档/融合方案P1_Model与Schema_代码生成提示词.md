...
> 实际代码实现与原 P1 设计存在以下偏差，均已在实际代码中落地。

### 8.1 Expert 模型改造 (§5.2)

| 改动点 | 原设计 | 实际实现 | 判断 |
|---|---|---|---|
| `department` 列 | **删除** | 删除 ORM 列定义，但保留 `@property department` 向前兼容（通过 `department_ref` 取值） | **实现更优**：保留向后兼容 |
| `category_id` FK `ondelete` | `SET NULL` | **`NO ACTION`** | **实现正确**：V7 将 `category_id` 设为 `NOT NULL`，`SET NULL` 会产生冲突 |
| relationship 命名 | `expert_department` | **`department_ref`** | 纯命名差异 |
| `department_name` @property | 未设计 | **新增** `@property department_name` 和 `@property category_name` | 实现更完整 |
| `idx_experts_department_id` 索引 | 在 `__table_args__` 中追加 | ⚠️ **ORM 模型未声明**（DB 层 V4 迁移已创建） | **代码需修复**：ORM `__table_args__` 应与 DB 实际索引一致，避免 `create_all()` 遗漏 |

### 8.2 Category 模型改造 (§5.3)

| 改动点 | 原设计 | 实际实现 | 判断 |
|---|---|---|---|
| `name` 的 `unique=True` | **移除**，改用部分索引 | ⚠️ **仍保留 `unique=True`** | **代码需修复**：V5 迁移已在 DB 层面用部分索引替代了全局 UNIQUE 约束，ORM 模型应与 DB 一致 |
| `__table_args__` 部分索引 | 添加 `uq_root_category_name` + `uq_sub_category_name` | ⚠️ **未添加**（仅注释写道"需要在 DDL 中定义"） | **代码需修复**：应与 V5 迁移同步 |
| `from sqlalchemy.sql import text` | 新增导入 | 未新增（因为部分索引未添加） | 随上一条修复时同步添加 |

### 8.3 Schema 改造 (§5.5)

| 改动点 | 原设计 | 实际实现 | 判断 |
|---|---|---|---|
| `ExpertBase.department` | 重命名为 `department_name` | **保留 `department` + 标注 `[DEPRECATED]`** | **实现更优**：向后兼容，避免 breaking change |
| `ExpertUpdate.department` | 重命名为 `department_name` | **保留 `department` + 标注 `[DEPRECATED]`** | 同上 |
| `FeaturedExpertItem.department` | 重命名为 `department_name` | **保留 `department` + 新增 `department_name` + `department_id`** | 同上 |
| `ExpertItem` 新增字段 | `department_name`, `department_id`, `category_name` | ✅ 已实现 | 一致 |
| ExpertDepartment Schema | 设计了 `ExpertDepartmentListResponse`（含 code/message/timestamp） | 保留了此 Schema 但实际 API 使用通用 `success_response()` 包装，不依赖它 | 无影响 |

### 8.4 设计正确、无需修改的点

- ✅ `ExpertDepartment` 模型：`name UNIQUE`、`category_id FK RESTRICT`、`synonyms JSONB default=list`、索引定义 — 全部一致
- ✅ `__init__.py` 注册：模型和 Schema 的注册均符合设计
- ✅ P1 不修改 `UserExpertSubscription`、`LiveSessionExpert`、`Tag`、`SessionTag`、`LiveRoomCategory` 的约束已遵守
