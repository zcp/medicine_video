...
> 实际代码实现与原 P3 设计存在以下偏差，均已在实际代码中落地。

### 8.1 Service 层实现差异

| 改动点 | 原设计 (§5.1) | 实际实现 | 判断 |
|---|---|---|---|
| **改动 B**：`_match_broad_category` | ExpertService 的 **async 实例方法**，内部访问 `self._root_categories` 缓存 | **模块级 sync 纯函数**，接收两个常量参数（`STANDARD_ROOT_CATEGORIES`、`DEPARTMENT_CATEGORY_MAP`） | **实现更优**：不访问数据库，async 无意义；纯函数更易测试 |
| **改动 C**：`_resolve_department_and_category` | 签名：`(department_name, category_id)`，内部用 `text()` SQL 查询 synonyms | 签名：`(department_text, category_id_raw, category_name_raw)`，使用会话级 `self._dept_cache` 批量加载科室做内存匹配 | **实现更优**：支持 `category_name` 显式传入（CSV 场景）；内存匹配避免每行 SQL 子查询 |
| **改动 D**：`_build_expert_item` | Service 层方法，手动填充 `category_name`/`department_name` | **未实现此方法**。改用 ORM `@property department_name` / `@property category_name` + Pydantic `from_attributes=True` 自动取值 | **实现更优**：更 DRY，利用框架能力而非手动拼接 |
| **改动 E**：`merge_departments` | 含 `SYNONYM_LIMIT=20`、自动升级 `is_verified`、`db.delete(source)` 物理删除 | 实现了核心流程（转移专家→追加同义词→物理删除），但 **未设同义词上限、未实现 verified 自动升级** | 差异可接受：同义词实践中不会超过 20 条；verified 升级应由管理员通过独立端点操作 |
| **改动 F**：`update_department_category` | 调用 CRUD 的 `update_department` + 手动 `sa_update(Expert)` | 直接修改 `dept.category_id` + 调用 CRUD `update_experts_category_by_department` | 一致，只是将同步 SQL 下沉到了 CRUD 层 |
| **改动 G**：`create_expert` 改造 | 调用 `_resolve_department_and_category` → `create_expert_raw(dict)` | 调用 `_resolve_department_and_category` → 修改 `ExpertCreate` 对象的 `department_id`/`category_id` → 复用已有 `crud.create_expert()` | **实现更优**：不引入新的 CRUD 函数，复用已有模式（参见 P2 同步记录 §8.2） |
| **常量定义**：`DEPARTMENT_CATEGORY_MAP` | 在文件头部、类定义之前 | ✅ 已实现（含 30+ 条别名） | 一致 |
| **`OTHER_CATEGORY_ID` 懒加载** | 在 `__init__` 中初始化 | 未使用单一变量。每次在需要时通过 `select(Category).where(name=='其他')` 查询 | 差异可接受：查询非常简单，且被 PostgreSQL 查询计划缓存 |

### 8.2 API 层实现差异

| 改动点 | 原设计 (§5.3) | 实际实现 | 判断 |
|---|---|---|---|
| **`format_expert_response`** | 应包含 `category_name` | ⚠️ **未包含 `category_name`**（虽然有 `department` 和 `department_name`） | **代码需修复**：ORM 已有 `@property category_name`，手动构造 dict 时遗漏 |
| **`get_featured_experts`** | 响应应包含 `category_name` | ⚠️ **未包含 `category_name`** | **代码需修复** |
| **auth 导入路径** | `from app.core.auth import get_current_user` | `from app.core.deps import get_current_user` | 实现反映真实项目结构，设计文档假设了不存在的路径 |
| **端点数量** | 设计 5 个 | 实现 **7 个**（新增 `merge_departments`、`update_department_category`） | 实现更完整 |
| **`expert_dept_admin_router`** | 在 P3 中新建 | ✅ 已实现 | 一致 |

### 8.3 分层职责实现差异

| 设计约定 | 实际实现 | 判断 |
|---|---|---|
| Service 层不直接调 `db.commit()`（§4.1） | `merge_departments` 和 `update_department_category` 内部需要 commit（多表写操作在同一事务中） | 设计已预见到这两个方法的特殊性（§4.3），实际实现符合设计例外 |
| API 层 `try/except` 异常处理模板 | ✅ 一致 | 已按设计实现 |
