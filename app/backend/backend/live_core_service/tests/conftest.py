...
# 先加载真实 .env（如果存在），保证本机/Docker 的端口、密码等配置生效；
# 后加载 .env.example 仅兜底（不覆盖已存在的环境变量）
load_dotenv(dotenv_path=os.path.join(base_dir, '.env'))
load_dotenv(dotenv_path=env_example_path)

# 测试环境统一使用测试密钥 my-key（与 .env.docker 保持一致）：
# .env / .env.example 中的占位符密钥会导致集成测试 JWT 签名验证失败（401）。
# 必须在导入 app 模块（settings 单例求值）之前设置。
...
import app.models.expert_departments  # noqa: F401 - register ExpertDepartment (experts.py 字符串引用)
...
async def setup_database():
    """设置测试数据库"""
    # 确保所有使用 Base 的模型在 create_all 前已注册（同执行上下文）
    import app.models  # noqa: F401
    import app.models.expert_departments  # noqa: F401
    ...
    async with engine.begin() as conn:
        ...
        # 创建所有表（live_core、content_management、liveroom_official_accounts 等）
...
# 添加项目根目录到 Python 路径（主机开发环境）
# Docker 容器中路径结构不同，用 try-except 安全降级
_host_root = os.path.join(os.path.dirname(__file__), '..', '..', '..')
sys.path.append(_host_root)

# 导入users服务的模型（主机环境）或安全跳过（Docker环境）
try:
...
def mock_current_user(test_user_id):
    ...
    return {
        ...
        "type": "access",  # H1 修复后要求 access type（否则 401）
...
def regular_user_token(regular_user_id: uuid.UUID, regular_user_role: str) -> str:
    ...
    # 使用与 .env.docker 一致的测试密钥 my-key
    # 不能用 os.getenv 读 .env.example（其值是占位符）
    secret_key = "my-key"
    algorithm = os.getenv("JWT_ALGORITHM", "HS256")
    token_data = {
        "user_id": str(regular_user_id),
        "type": "access",  # H1 修复后要求 access type
...
def another_user_token(another_user_id: uuid.UUID) -> str:
    ...
    # 统一使用 conftest 顶部设置的测试密钥 my-key
    # （移除 load_dotenv(override=True)：其会用 .env.example 占位密钥覆盖环境变量，导致签名与后端不一致）
    secret_key = "my-key"
    algorithm = os.getenv("JWT_ALGORITHM", "HS256")
    token_data = {
...
def expired_token(regular_user_id: uuid.UUID, regular_user_role: str) -> str:
    ...
    token_data = {
        "user_id": str(regular_user_id),
        "type": "access",  # H1 修复后要求 access type（过期校验仍生效）
        "role": regular_user_role,
        "sub": str(regular_user_id),
        "email": "regular@example.com",
