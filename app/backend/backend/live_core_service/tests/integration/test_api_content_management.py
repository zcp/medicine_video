        ...
        async for client in async_client:
            async for db in db_session:
                ...
                response = await client.get(
                    "/api/v1/content/tags?include_inactive=true",
...
class TestTagResolveAPI:
    """标签 resolve（解析或创建）API 测试（V2 阶段 2）"""

    @pytest.mark.asyncio
    async def test_resolve_create_and_reuse(
        self,
        async_client: httpx.AsyncClient,
        db_session: AsyncSession,
        regular_user_token: str,
        ...
        """resolve：首次创建（source=user, created_by=当前用户），再次同名复用同一 id"""
        async for client in async_client:
            async for db in db_session:
                name = f"resolve自建标签_{uuid4().hex[:8]}"
                ...
                assert data1["data"]["created"] is True
                assert data1["data"]["source"] == "user"
                tag_id = data1["data"]["id"]

                # 落库校验：source=user 且 created_by=当前用户
                result = await db.execute(select(Tag).where(Tag.id == UUID(tag_id)))
                row = result.scalar_one_or_none()
                assert row is not None
                assert row.source == "user"
                assert row.created_by == regular_user_id

                # Act 2: 同名再 resolve → 复用
                ...
                assert data2["data"]["id"] == tag_id

    @pytest.mark.asyncio
    async def test_resolve_soft_deleted_rejected(
        self,
        async_client: httpx.AsyncClient,
        db_session: AsyncSession,
        regular_user_token: str
    ):
        """resolve：软删同名不复活 → 400（4001）"""
        async for client in async_client:
            async for db in db_session:
                name = f"resolve软删标签_{uuid4().hex[:8]}"
                soft = Tag(id=uuid4(), name=name, is_active=False)
                db.add(soft)
                await db.commit()
    ...
    @pytest.mark.asyncio
    async def test_resolve_unauthorized(self, async_client: httpx.AsyncClient):
        """resolve：未登录 → 401"""
        async for client in async_client:
            response = await client.post(
                "/api/v1/content/tags/resolve",
    ...
    @pytest.mark.asyncio
    async def test_resolve_content_safety_blocked(
        self,
        async_client: httpx.AsyncClient,
        regular_user_token: str,
        monkeypatch
    ):
        """resolve：内容安全拦截 → 422（2005），不落库"""
        async for client in async_client:
            from unittest.mock import AsyncMock
            from app.content_safety.exceptions import ContentSafetyBlockedException
            from app.services.content_management_service import check_scene_fields as _orig  # noqa: F401

            async def _blocked(*args, **kwargs):
                raise ContentSafetyBlockedException("违规拦截测试")
    ...
    async for client in async_client:
        async for db in db_session:
            ...
            response = await client.post(
                ...
                json={"category_ids": [str(c1.id), str(c2.id)], "primary_category_id": str(c1.id), "mode": "replace"},
                headers={"Authorization": f"Bearer {admin_user_token}"},
            )
            assert response.status_code == 200
            data = response.json()
...
async def test_api_set_live_room_categories_permission_denied(
    ...
    async for client in async_client:
        async for db in db_session:
            ...
            response = await client.post(
                f"/api/v1/admin/rooms/{room.id}/categories",
                json={"category_ids": [str(c1.id)], "primary_category_id": str(c1.id), "mode": "replace"},
                headers={"Authorization": f"Bearer {regular_user_token}"},
            )
            assert response.status_code == 403
...
async def test_api_set_live_room_categories_invalid_category_id(
    ...
    async for client in async_client:
        async for db in db_session:
            ...
            response = await client.post(
                f"/api/v1/admin/rooms/{room.id}/categories",
                json={"category_ids": [str(fake_cat_id)], "primary_category_id": str(fake_cat_id), "mode": "replace"},
                headers={"Authorization": f"Bearer {admin_user_token}"},
            )
            assert response.status_code == 400
