...
# ==================== Expert Category API 测试（增量） ====================


@pytest.mark.asyncio
async def test_api_create_expert_with_category_id(
    async_client, db_session, admin_user_token
):
    """测试创建专家时传入 category_id，响应包含 category_id 和 category_name"""
    async for db in db_session:
        ...
        async for client in async_client:
            ...
            assert response.status_code == 201
            data = response.json()
            assert data.get("data", {}).get("category_id") == str(cat.id)
            assert data.get("data", {}).get("category_name") == cat.name
            break
        break


@pytest.mark.asyncio
async def test_api_get_expert_detail_includes_category(
    async_client, db_session
):
    """测试获取专家详情时响应包含 category_id 和 category_name"""
    async for db in db_session:
        from app.models.content_management import Category
        cat = Category(id=uuid.uuid4(), name=f"分类_{uuid.uuid4().hex[:8]}", sort_order=0, is_active=True)
        db.add(cat)
        ...
        expert = await create_test_expert_db(db, category_id=cat.id)
...
@pytest.mark.asyncio
async def test_api_get_featured_experts_includes_category_id(
    async_client, db_session
):
    """测试推荐专家列表响应包含 category_id"""
    async for db in db_session:
        from app.models.content_management import Category
        cat = Category(id=uuid.uuid4(), name=f"分类_{uuid.uuid4().hex[:8]}", sort_order=0, is_active=True)
        db.add(cat)
        ...
        await create_test_expert_db(db, category_id=cat.id, is_featured=True,
                                      name=f"推荐专家_{uuid.uuid4().hex[:6]}")

        async for client in async_client:
            response = await client.get("/api/v1/featured-experts?limit=50")
            assert response.status_code == 200
            data = response.json()
            assert "data" in data
            assert isinstance(data["data"], list)
            # 至少有记录包含 category_id（韧性断言，不依赖精确计数）
            category_ids = [item.get("category_id") for item in data["data"] if item.get("category_id")]
...
@pytest.mark.asyncio
async def test_api_get_experts_public_list_filter_by_category_id(
    async_client, db_session
):
    """测试公开专家列表按 category_id 过滤"""
    async for db in db_session:
        from app.models.content_management import Category
        suffix = uuid.uuid4().hex[:8]
        cat1 = Category(id=uuid.uuid4(), name=f"分类1_{suffix}", sort_order=0, is_active=True)
        cat2 = Category(id=uuid.uuid4(), name=f"分类2_{suffix}", sort_order=1, is_active=True)
        db.add_all([cat1, cat2])
        await db.commit()

        await create_test_expert_db(db, category_id=cat1.id, name=f"专家_{suffix}_1", is_active=True)
        await create_test_expert_db(db, category_id=cat2.id, name=f"专家_{suffix}_2", is_active=True)

        async for client in async_client:
            response = await client.get(f"/api/v1/experts?category_id={cat1.id}&page=1&size=100")
            assert response.status_code == 200
            data = response.json()
            items = data.get("data", {}).get("items", [])
            for item in items:
                assert item.get("category_id") == str(cat1.id)
            break
        break


@pytest.mark.asyncio
async def test_api_get_experts_public_list_invalid_category_id_400(
    async_client
):
    """测试公开专家列表传入无效 category_id 格式返回 400"""
    async for client in async_client:
        response = await client.get("/api/v1/experts?category_id=not-a-valid-uuid")
        assert response.status_code == 400
        data = response.json()
...
@pytest.mark.asyncio
async def test_api_get_experts_admin_list_filter_by_category_id(
    async_client, db_session, admin_user_token
):
    """测试 Admin 专家列表按 category_id 过滤"""
    async for db in db_session:
        from app.models.content_management import Category
        suffix = uuid.uuid4().hex[:8]
        cat = Category(id=uuid.uuid4(), name=f"分类_{suffix}", sort_order=0, is_active=True)
        db.add(cat)
        await db.commit()

        await create_test_expert_db(db, category_id=cat.id, name=f"专家_{suffix}", is_active=True)
        await create_test_expert_db(db, name=f"其他专家_{suffix}", is_active=True)

        async for client in async_client:
            response = await client.get(
                f"/api/v1/admin/experts?category_id={cat.id}&page=1&size=100",
                headers={"Authorization": f"Bearer {admin_user_token}"}
            )
            assert response.status_code == 200
            data = response.json()
            items = data.get("data", {}).get("items", [])
            for item in items:
                assert item.get("category_id") == str(cat.id)
            break
        break


@pytest.mark.asyncio
async def test_api_get_experts_admin_list_invalid_category_id_400(
    async_client, admin_user_token
):
    """测试 Admin 专家列表传入无效 category_id 格式返回 400"""
    async for client in async_client:
        response = await client.get(
            "/api/v1/admin/experts?category_id=not-a-valid-uuid",
            headers={"Authorization": f"Bearer {admin_user_token}"}
        )
        assert response.status_code == 400
        data = response.json()
...
@pytest.mark.asyncio
async def test_api_update_expert_category_id(
    async_client, db_session, admin_user_token
):
    """测试更新专家 category_id"""
    async for db in db_session:
        from app.models.content_management import Category
        suffix = uuid.uuid4().hex[:8]
        cat1 = Category(id=uuid.uuid4(), name=f"分类A_{suffix}", sort_order=0, is_active=True)
        cat2 = Category(id=uuid.uuid4(), name=f"分类B_{suffix}", sort_order=1, is_active=True)
        db.add_all([cat1, cat2])
        await db.commit()

        expert = await create_test_expert_db(db, category_id=cat1.id)
        ...
        async for client in async_client:
            ...
            assert response.status_code == 200
            data = response.json()
            assert data.get("data", {}).get("category_id") == str(cat2.id)
            assert data.get("data", {}).get("category_name") == cat2.name
            break
        break