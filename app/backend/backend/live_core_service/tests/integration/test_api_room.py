...
async def test_create_room_success(
    ...
    async for client in async_client:
        ...
        assert data["stream_key"].startswith("streamkey_")
        assert uuid.UUID(data["id"])  # 验证ID是有效的UUID
        break

# ← 新增：测试被禁播用户（can_stream=false）创建房间返回403（PR 1B）
@pytest.mark.asyncio
async def test_create_room_banned_user_forbidden(async_client):
    """被禁播用户（can_stream=false）创建房间 → 403 + code=3002"""
    import time
    import jwt

    secret_key = "my-key"  # 与 conftest 测试密钥一致
    algorithm = "HS256"
    banned_token = jwt.encode(
        {
    ...
    room_data = {"title": "被禁播用户的房间", "description": "不应创建成功"}
    ...
    async for client in async_client:
        ...
        assert "你已被禁止开播" in response_json["message"]
        break


# ← 新增：测试未知角色 token 创建房间返回403（PR 1B 防御）
@pytest.mark.asyncio
async def test_create_room_unknown_role_forbidden(async_client):
    """未知角色（role=""）+ can_stream=true → 403 + code=3002（防御）"""
    import time
    import jwt
    ...
    room_data = {"title": "访客 token 的房间", "description": "不应创建成功"}
    ...
    async for client in async_client:
        ...
        response_json = response.json()
        assert response_json["code"] == 3002  # 权限不足错误码
        assert "无开播权限" in response_json["message"]
        break
...
async def test_get_rooms_success_as_owner(
    ...
    """
    测试成功获取房间列表（登录用户，可以看到Public+自己的房间，S3场景）
    """
    # ===== Arrange (准备) =====
    headers = {"Authorization": f"Bearer {regular_user_token}"}
    # ← 修改：在循环外部获取room_id，避免重复获取
    ...
    async for client in async_client:
        ...
        room_ids = [item["id"] for item in data["items"]]
        assert str(user_room_id) in room_ids  # 自己的Private房间应该可见
        break
...
# ← 修改：匿名访问 Private 房间 → 200（unlisted 语义，2026-08-11 V1.1 决策 D1）
@pytest.mark.asyncio
async def test_get_room_private_anonymous(
    async_client, 
    private_room
):
    """匿名用户访问 Private 房间 → 200（unlisted：持链可读，不再 404 伪装）"""
    # ===== Arrange (准备) =====
    # private_room已在Fixture中创建
    ...
    async for client in async_client:
        ...
        # ===== Assert (断言) =====
        assert response.status_code == 200  # unlisted：匿名持链可读
        response_json = response.json()
        assert response_json["code"] == 200
...
async def test_get_room_private_as_non_owner(
    ...
    """非 Owner 访问他人 Private 房间 → 200（unlisted：持链可读，不再 404 伪装）"""
    # ===== Arrange (准备) =====
    ...
    async for client in async_client:
        ...
        # ===== Assert (断言) =====
        assert response.status_code == 200  # unlisted：他人持链可读
        response_json = response.json()
        assert response_json["code"] == 200
        assert response_json["data"]["id"] == str(room_id)
        break
