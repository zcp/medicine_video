...
async def test_check_brand_references_no_refs(db_session):
    ...
    async for db in db_session:
        ...
        # ===== Act (执行) =====
        topic_count, room_count, featured_count = await crud.check_brand_references(db, brand.id)
        
        # ===== Assert (断言) =====
        assert topic_count == 0
        assert room_count == 0
        assert featured_count == 0  # 数据关联治理（P0-1）：新增焦点图引用计数
        break
...
async def test_check_brand_references_with_rooms(db_session):
    ...
    async for db in db_session:
        ...
        # ===== Act (执行) =====
        topic_count, room_count, featured_count = await crud.check_brand_references(db, brand1.id)
