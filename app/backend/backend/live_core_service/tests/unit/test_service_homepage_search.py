...
async def test_get_featured_content_list_admin_paginated_success(service, mock_db, mock_featured_content):
    ...
    with patch('app.services.homepage_search_service.crud') as mock_crud:
        ...
        mock_crud.get_featured_content_list_paginated.assert_called_once_with(
            mock_db, page=1, size=10, q=None, search_type=None, is_active=None, status=None
        )
...
async def test_get_featured_content_list_admin_paginated_passes_q_and_search_type(service, mock_db, mock_featured_content):
    ...
    with patch('app.services.homepage_search_service.crud') as mock_crud:
        ...
        assert result["code"] == 200
        mock_crud.get_featured_content_list_paginated.assert_called_once_with(
            mock_db, page=2, size=5, q="关键词", search_type="name", is_active=None, status=None
        )
...
# ==================== 焦点图时间区间校验（start_at ≤ end_at）增量测试 ====================

def _aware_dt(y, m, d):
    """构造带 UTC 时区的 datetime，避免 naive/aware 比较错误"""
    return datetime(y, m, d, tzinfo=timezone.utc)


@pytest.mark.asyncio
async def test_create_featured_content_end_before_start_raises(service, mock_db):
    """测试创建焦点图 - 下线时间早于上线时间应被拦截"""
    admin_id = uuid.uuid4()
    admin_role = "ADMIN"
    content_data = FeaturedContentCreate(
        title="乱序焦点图",
        image_url="https://example.com/bad.jpg",
        start_at=_aware_dt(2026, 9, 10),
        end_at=_aware_dt(2026, 9, 1),
    )
    ...
    with patch('app.services.homepage_search_service.crud') as mock_crud:
        ...
        with pytest.raises(InvalidParameterException):
            await service.create_featured_content(mock_db, content_data, admin_id, admin_role)

        mock_crud.create_featured_content.assert_not_called()


@pytest.mark.asyncio
async def test_create_featured_content_end_equal_start_success(service, mock_db, mock_featured_content):
    """测试创建焦点图 - 下线时间等于上线时间（同一天）应允许"""
    admin_id = uuid.uuid4()
    admin_role = "ADMIN"
    content_data = FeaturedContentCreate(
        title="同日焦点图",
        image_url="https://example.com/ok.jpg",
        start_at=_aware_dt(2026, 9, 1),
        end_at=_aware_dt(2026, 9, 1),
    )
...
@pytest.mark.asyncio
async def test_update_featured_content_end_before_existing_start_raises(service, mock_db, mock_featured_content):
    """测试更新焦点图 - 仅改下线时间，使其早于现有上线时间应被拦截"""
    admin_id = uuid.uuid4()
    admin_role = "ADMIN"
    content_id = uuid.uuid4()
    update_data = FeaturedContentUpdate(end_at=_aware_dt(2026, 8, 1))

    existing = MagicMock()
    existing.start_at = _aware_dt(2026, 9, 1)
    existing.end_at = _aware_dt(2026, 10, 1)

    with patch('app.services.homepage_search_service.crud') as mock_crud:
        mock_crud.get_featured_content_by_id = AsyncMock(return_value=existing)

        with pytest.raises(InvalidParameterException):
            await service.update_featured_content(mock_db, content_id, update_data, admin_id, admin_role)

        mock_crud.update_featured_content.assert_not_called()


@pytest.mark.asyncio
async def test_update_featured_content_valid_window_success(service, mock_db, mock_featured_content):
    """测试更新焦点图 - 时间区间合法时正常更新（仅改 end_at，且晚于现有 start_at）"""
    admin_id = uuid.uuid4()
    admin_role = "ADMIN"
    content_id = uuid.uuid4()
    update_data = FeaturedContentUpdate(end_at=_aware_dt(2026, 12, 1))
    ...
    with patch('app.services.homepage_search_service.crud') as mock_crud:
        ...
        assert result["code"] == 200
        mock_crud.update_featured_content.assert_called_once()


@pytest.mark.asyncio
async def test_delete_featured_content_success(service, mock_db, mock_featured_content):
    """测试删除焦点图 - 成功（物理删除）"""
    # ===== Arrange (准备) =====
    admin_id = uuid.uuid4()
    admin_role = "ADMIN"
    ...
    with patch('app.services.homepage_search_service.crud') as mock_crud:
        ...
        # 物理删除：必须传 soft_delete=False
        mock_crud.delete_featured_content.assert_called_once_with(
            mock_db, content_id, soft_delete=False
        )


@pytest.mark.asyncio
async def test_delete_featured_content_not_found(service, mock_db):
    """测试删除焦点图 - 不存在"""
    # ===== Arrange (准备) =====
    admin_id = uuid.uuid4()
    admin_role = "ADMIN"
    ...
    with patch('app.services.homepage_search_service.crud') as mock_crud:
        ...
        # ===== Act & Assert (执行和断言) =====
        with pytest.raises(NotFoundException):
            await service.delete_featured_content(mock_db, content_id, admin_id, admin_role)
...
def test_select_host_without_expert(service):
    """测试Host选择逻辑 - 无场次专家（P1 行为：fallback 到创建者，无 owner 时 name='用户'）"""
    # ===== Arrange (准备) =====
    room_data = {
        'expert_id': None,
        'expert_name': None,
    ...
    # ===== Assert (断言) =====
    # P1 行为：无专家且无创建者 → 返回 HomepageHostInfo，name 兜底为"用户"
    assert host is not None
    assert host.expert_id is None
    assert host.user_id is None
    assert host.name == '用户'
    assert host.title is None
    assert host.hospital is None


def test_select_host_without_expert_fallback_to_owner(service):
    """测试Host选择逻辑 - 无场次专家，fallback 到创建者公开资料（P1 行为）"""
    # ===== Arrange (准备) =====
    owner_id = uuid.uuid4()
    room_data = {
        'expert_id': None,
    ...
    user_profile_map = {
        str(owner_id): {'nickname': '创建者昵称', 'username': 'creator', 'avatar_url': 'http://a/b.png'},
    }

    # ===== Act (执行) =====
    host = service._select_host(room_data, user_profile_map)

    # ===== Assert (断言) =====
    assert host is not None
    assert host.expert_id is None
    assert host.user_id == owner_id
    assert host.name == '创建者昵称'  # nickname 优先
    assert host.title is None
    assert host.hospital is None
    assert host.avatar_url == 'http://a/b.png'
