...
class TestTopicManagement:
    ...
    async def test_delete_topic_success(
        ...
        # Mock 数据关联治理（P0-1）：删除专题时软下线指向该专题的焦点图
        ...
        mock_disable_featured.assert_called_once_with(mock_db, "topic", topic_id)
        assert result == mock_topic
...
class TestCategoryManagement:
    ...
    async def test_create_category_topic_not_found(
        ...
        category_in = TopicCategoryCreate(name="Test Category", sort_order=10)
    ...
    async def test_update_category_success(
        ...
        update_data = TopicCategoryUpdate(name="Updated Category")
    ...
    async def test_update_category_not_found(self, mocker):
        ...
        2. 抛出 TopicCategoryNotFoundException
        """
        # Arrange
        mock_db = mocker.Mock()
        user_id = uuid.uuid4()
        ...
        update_data = TopicCategoryUpdate(name="New Name")
        ...
        service = TopicService(mock_db)
        role = "REGULAR"  # ← 新增：权限参数
        with pytest.raises(TopicCategoryNotFoundException):
