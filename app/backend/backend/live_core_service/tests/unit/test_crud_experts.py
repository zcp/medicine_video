...
class TestExpertCRUD:
    ...
    async def test_create_expert_success(self, db_session):
        ...
        async for db in db_session:
            ...
            assert expert.name == "方书强"
            assert expert.title == "主任医师教授"
            assert expert.hospital == "北京大学人民医院"
            assert expert.expertise_areas == "肝胆脾胃病,微创外科"
...
class TestUserExpertSubscriptionCRUD:
    ...
    @pytest.mark.asyncio
    async def test_count_followers_by_expert(self, db_session):
        """测试统计专家关注者数量"""
        async for db in db_session:
            expert = await create_test_expert(db)
            user_id1 = uuid.uuid4()
            ...
            await crud_experts.create_subscription(db, user_id1, expert.id)
            await crud_experts.create_subscription(db, user_id2, expert.id)
            await db.commit()

            count = await crud_experts.count_followers_by_expert(db, expert.id)
            assert count == 2
            break

    @pytest.mark.asyncio
    async def test_count_followers_by_expert_zero(self, db_session):
        """测试统计无关注者"""
        async for db in db_session:
            expert = await create_test_expert(db)
            await db.commit()
...
# ==================== Expert CRUD Category 测试（增量） ====================

async def create_test_category(db: AsyncSession, **kwargs) -> "Category":
    """创建测试分类的辅助函数"""
...
@pytest.mark.asyncio
async def test_create_expert_with_category_id(db_session):
    """测试创建专家时传入 category_id"""
    async for db in db_session:
        cat = await create_test_category(db)
        await db.commit()

        expert = await create_test_expert(db, category_id=cat.id)
        await db.commit()
        await db.refresh(expert)

        assert expert.category_id == cat.id
        # 验证 joinedload 正确加载 category 关系
        fetched = await crud_experts.get_expert(db, expert.id)
...
@pytest.mark.asyncio
async def test_get_expert_eager_loads_category(db_session):
    """测试 get_expert 使用 joinedload 预加载 category 关系"""
    async for db in db_session:
        cat = await create_test_category(db)
        await db.commit()
        expert = await create_test_expert(db, category_id=cat.id)
        ...
        fetched = await crud_experts.get_expert(db, expert.id)
        assert fetched is not None
        # category 关系应已被 joinedload 预加载
        assert fetched.category is not None
        assert fetched.category.id == cat.id
        assert fetched.category.name == cat.name
        break


@pytest.mark.asyncio
async def test_get_experts_public_list_filter_by_category_id(db_session):
    """测试公开专家列表按 category_id 过滤"""
    async for db in db_session:
        cat1 = await create_test_category(db)
        cat2 = await create_test_category(db)
        await db.commit()
        suffix = uuid.uuid4().hex[:8]
        expert1 = await create_test_expert(db, category_id=cat1.id, name=f"专家_{suffix}_1", is_active=True)
        expert2 = await create_test_expert(db, category_id=cat2.id, name=f"专家_{suffix}_2", is_active=True)
        await db.commit()

        experts, total = await crud_experts.get_experts_public_list(db, page=1, size=50, category_id=cat1.id)
        expert_ids = {e.id for e in experts}
        assert expert1.id in expert_ids
        assert expert2.id not in expert_ids
        break


@pytest.mark.asyncio
async def test_get_experts_public_list_filter_by_disabled_category(db_session):
    """阶段5：公开专家列表按已禁用分类过滤 → 400（不再静默返回空）"""
    async for db in db_session:
        cat = await create_test_category(db, is_active=False)
        await db.commit()
        suffix = uuid.uuid4().hex[:8]
        await create_test_expert(db, category_id=cat.id, name=f"专家_{suffix}", is_active=True)
        await db.commit()

        from app.exceptions import InvalidParameterException
        with pytest.raises(InvalidParameterException) as exc_info:
            await crud_experts.get_experts_public_list(db, page=1, size=50, category_id=cat.id)
        assert "分类不存在或已停用" in str(exc_info.value)
        break


@pytest.mark.asyncio
async def test_get_experts_multi_and_total_filter_by_category_id(db_session):
    """测试 Admin 专家列表按 category_id 过滤"""
    async for db in db_session:
        cat = await create_test_category(db)
        await db.commit()
        suffix = uuid.uuid4().hex[:8]
        expert1 = await create_test_expert(db, category_id=cat.id, name=f"专家_{suffix}_1")
        expert2 = await create_test_expert(db, name=f"专家_{suffix}_2")
        await db.commit()

        experts, total = await crud_experts.get_experts_multi_and_total(
            db, skip=0, limit=50, category_id=cat.id
        )
        expert_ids = {e.id for e in experts}
        assert expert1.id in expert_ids
        assert expert2.id not in expert_ids
