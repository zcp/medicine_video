...
# 管理端公众号 CRUD：挂载 prefix=/admin -> /admin/official_accounts, /admin/official_accounts/{account_id}
admin_official_accounts_router = APIRouter(tags=["直播间与公众号关联-管理端"])

# 公开：按房间查公众号 挂载 prefix=/rooms -> /rooms/{room_id}/official_accounts
rooms_official_accounts_router = APIRouter(tags=["直播间与公众号关联-公开"])

# 管理端：房间-公众号绑定 挂载 prefix=/admin/rooms -> /admin/rooms/{room_id}/official_accounts
...
@admin_official_accounts_router.get("/official-accounts/{account_id}/rooms", summary="按公众号分页查直播间列表（管理端）")
