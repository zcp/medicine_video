...
async def get_recommendations(
    keyword: Optional[str] = Query(None, description="搜索关键词（可选，为空时返回全局热词）"),
    ...
    - keyword 为空：返回全局热门搜索词
    - keyword 非空：返回包含匹配的推荐词
    - 登录用户：其搜索历史中的词获得个性化 boost
    """
    try:
        user_id = None
        if current_user:
        ...
        items = await search_crud.get_recommendations(db, keyword=keyword, user_id=user_id, limit=limit)
