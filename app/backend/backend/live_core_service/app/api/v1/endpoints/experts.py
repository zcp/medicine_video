...
from app.schemas.experts import (
    ...
    FollowedExpertsResponse, SessionExpertItem, ExpertSelfUpdate,
)
...
def format_expert_response(expert, request: Request = None) -> dict:
    ...
    V7 后 department 列已删除，科室名通过 Expert.department property
    （→ department_ref.name）自动获取。保留 department 键向后兼容，
    同时提供 department_name 作为推荐字段。
    """
    # 通过 @property department 自动取值（兼容 ORM 和 Pydantic 对象）
    department_name = getattr(expert, 'department', None)
    department_id_val = (
        str(getattr(expert, 'department_id', None))
        if getattr(expert, 'department_id', None) else None
    )
    ...
    data = {
        ...
        "department": department_name,           # 向后兼容
        "department_name": department_name,       # 新字段（推荐）
        "department_id": department_id_val,       # 新字段
        ...
        experts_data = [
            {
                ...
                "title": expert.title,
                "hospital": expert.hospital,
                "department": expert.department_name,     # 向后兼容，值同 department_name
                "department_name": expert.department_name,
                "department_id": str(expert.department_id) if expert.department_id else None,
                "expertise_areas": expert.expertise_areas,
...
async def get_experts_public_list(
    ...
    category_id: Optional[str] = Query(default=None, description="专家主专业分类ID筛选（按 experts.category_id 过滤）"),
    keyword: Optional[str] = Query(default=None, description="关键词（姓名/医院/科室/简介）"),
    department: Optional[str] = Query(default=None, description="按展示用细分科室文本筛选；不作为主分类筛选来源"),
    hospital: Optional[str] = Query(default=None, description="按医院筛选"),
    is_active: Optional[bool] = Query(default=None, description="按启用状态筛选，不传默认仅启用专家"),
    sort: Optional[str] = Query(default=None, description="排序规则，格式 field:direction，如 name:asc"),
    db: AsyncSession = Depends(get_db),
):
    """
    获取公开专家列表（分页）

    无需登录即可访问，默认仅返回已启用的专家。
    支持按分类ID、关键词、科室、医院筛选，支持自定义排序。
    """
    # 校验 category_id 为合法 UUID
    ...
    if category_id:
        try:
            ...
            return JSONResponse(
                status_code=400,
                content=error_response(code=4001, message="无效的分类ID格式")
            )

    try:
        service = ExpertService(db)
        result = await service.get_experts_public_list(
            page=page,
            size=size,
        ...
        logger.error(f"获取公开专家列表异常: {str(e)}")
        return JSONResponse(
            status_code=500,
            content=error_response(code=1002, message="数据库操作错误")
        )
...
# ==================== PR 5: 专家自认领 /experts/me ====================
# 注意：/me 路由必须在 /{expert_id} 之前注册，避免被通配符匹配

@experts_public_router.get("/me")
async def get_my_expert(
    request: Request,
    ...
    try:
        service = ExpertService(db)
        expert = await service.get_my_expert(current_user_id=user_id)
        if expert is None:
            return success_response(data=None)
        from app.crud import experts as crud_experts
        expert_orm = await crud_experts.get_expert_by_user_id(db, user_id)
        if expert_orm:
            return success_response(data={
                "id": str(expert_orm.id),
                "name": expert_orm.name,
                "title": expert_orm.title,
                "hospital": expert_orm.hospital,
                "department_name": getattr(expert_orm, "department_name", None),
                "expertise_areas": expert_orm.expertise_areas,
                "bio": expert_orm.bio,
                "avatar_url": expert_orm.avatar_url,
            })
        return success_response(data={"user_id": str(user_id), "bound": True})
    except Exception as e:
        logger.error(f"GET /experts/me 异常: {e}")
        return JSONResponse(status_code=500, content=error_response(code=1002, message="数据库操作错误"))


@experts_public_router.patch("/me")
async def patch_my_expert(
    request: Request,
    body: "ExpertSelfUpdate",
    current_user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    user_id = uuid.UUID(current_user["user_id"])
    try:
        service = ExpertService(db)
        await service.update_my_expert(current_user_id=user_id, expert_data=body)
        return success_response(message="专家信息已更新")
    except Exception as e:
        logger.error(f"PATCH /experts/me 异常: {e}")
        return JSONResponse(status_code=500, content=error_response(code=1002, message="数据库操作错误"))
