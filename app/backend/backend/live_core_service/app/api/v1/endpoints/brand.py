...
brand_admin_router = APIRouter()
brand_service = BrandService()


# ==================== Brands API端点 (7个) ====================

@brand_public_router.get("/brands")
...
@brand_public_router.get("/brands/{brand_id}/content")
...
@brand_public_router.get("/brands/{brand_id}/rooms")
async def get_brand_rooms_public(
    brand_id: UUID = Path(..., description="品牌ID"),
    ...
    """获取品牌关联直播间列表（Public + Optional Auth）"""
    current_user_id = UUID(current_user["user_id"]) if current_user else None
    role = current_user.get("role", "REGULAR").upper() if current_user else None
    user_id_for_logging = str(current_user_id)[:8] if current_user_id else "anonymous"
    ...
    try:
        ...
        # 查询关联直播间（分页）
        rooms, total = await crud_brand.get_brand_rooms_paginated(db, brand_id, page, size)

        # 批量聚合卡片字段（复用 room_card_service：状态/专家/创建者，无专家时回退创建者）
        room_card_map = {}
        if rooms:
            try:
                room_ids = [UUID(r["room_id"]) if not isinstance(r["room_id"], UUID) else r["room_id"] for r in rooms]
                ...
                logger.warning(f"品牌直播间卡片聚合失败，跳过附加字段: {e}")

        items = []
        for r in rooms:
            card = room_card_map.get(str(r["room_id"])) or {}
            items.append({
        ...
        logger.info(f"获取品牌关联直播间(公开): brand_id={str(brand_id)[:8]}, 返回{len(items)}条，总数{total}")
        ...
        logger.error(f"获取品牌直播间异常: user_id={user_id_for_logging}, error={type(e).__name__}, detail={e}")
        return JSONResponse(
            status_code=500,
            content=error_response(code=1002, message="内部服务器错误")
        )
...
@brand_admin_router.post("/brands")
...
@brand_admin_router.get("/brands/{brand_id}")
...
@brand_admin_router.patch("/brands/{brand_id}")
...
@brand_admin_router.delete("/brands/{brand_id}")
...
@brand_admin_router.post("/brands/{brand_id}/logo")
...
@brand_admin_router.post("/brands/{brand_id}/topics")
...
@brand_admin_router.delete("/brands/{brand_id}/topics/{topic_id}")
...
@brand_admin_router.get("/brands/{brand_id}/topics")
...
@brand_admin_router.get("/brands/{brand_id}/rooms")
...
@brand_admin_router.get("/rooms/{room_id}/brands")
...
@brand_public_router.get("/rooms/{room_id}/brands")
