...
async def get_recommendations(
    ...
    - keyword 为空时：返回全局热词（按搜索次数降序）
    - keyword 非空时：返回 ILIKE 包含匹配的推荐词
    - 登录用户：其搜索历史中的词获得 +50 个性化 boost
    """
    # 获取用户历史词集（用于个性化 boost）
    user_history_keywords: set = set()
    if user_id:
        hist_stmt = (
        ...
        user_history_keywords = {row[0] for row in hist_result.all()}
    ...
    if keyword and keyword.strip():
        ...
        stmt = (
            select(
                SearchKeywordStats.keyword_norm,
                SearchKeywordStats.keyword,
                SearchKeywordStats.total_count
            )
            .where(SearchKeywordStats.keyword_norm.ilike(pattern))
            .order_by(SearchKeywordStats.total_count.desc())
            .limit(limit * 2)
        )
    else:
        stmt = (
            ...
            .where(SearchKeywordStats.total_count >= 1)
            .order_by(SearchKeywordStats.total_count.desc())
            .limit(limit * 2)
        )
    ...
    # 计算评分：基础分 = search_count，个性化 boost = +50
    scored = []
    for row in rows:
        norm, orig_keyword, count = row[0], row[1], row[2]
        score = float(count)
        if norm in user_history_keywords:
            score += 50.0
        display_keyword = orig_keyword if orig_keyword else norm
        scored.append({"keyword": display_keyword, "score": score})
