...
from app.models.homepage_search import FeaturedContent  # 焦点图（数据关联治理 P0-1）
...
async def get_brands(
    ...
    """
    # 构建查询
    # 次级排序：sort_order 相同时按创建时间稳定排序，避免顺序随机
    stmt = select(Brand).where(Brand.is_active == True).order_by(Brand.sort_order, Brand.created_at)
...
async def get_brands_paginated(
    ...
    # 第二次查询：获取当前页数据
    # 次级排序：sort_order 相同时按创建时间稳定排序，避免顺序随机
    stmt = select(Brand).order_by(Brand.sort_order, Brand.created_at)
    if filters:
...
async def delete_brand(
    ...
    try:
        ...
        if hard_delete:
            ...
            # 数据关联治理（P0-1，D3 定稿）：软删品牌连带软下线指向它的焦点图
            # （与品牌状态一致，避免"品牌没了焦点图还在播"；焦点图记录保留，品牌恢复后运营可手动恢复）
            from app.crud.homepage_search import disable_featured_content_by_target
            await disable_featured_content_by_target(db, "brand", brand_id)
...
async def check_brand_references(
    ...
    Returns:
        (专题引用数, 直播间引用数, 焦点图引用数)
        - 数据关联治理（P0-1，D2 定稿）：新增焦点图引用计数，
          硬删品牌时有焦点图引用即拒绝（与 BrandTopic/BrandRoom 同语义）
    """
    # 查询专题引用数
    ...
    # 查询焦点图引用数（FeaturedContent.target_id 无外键，应用层计数）
    featured_count_stmt = select(func.count()).select_from(FeaturedContent).where(
        FeaturedContent.target_type == "brand",
        FeaturedContent.target_id == brand_id,
    )
    featured_count = await db.scalar(featured_count_stmt) or 0
    
    logger.info(f"品牌引用检查: id={str(brand_id)[:8]}, 专题:{topic_count}, 直播间:{room_count}, 焦点图:{featured_count}")
    return topic_count, room_count, featured_count
...
async def get_room_brands(
    ...
    stmt = (
        ...
        # 次级排序：sort_order 相同时按创建时间稳定排序，避免顺序随机
        .order_by(Brand.sort_order, Brand.created_at)
    )
