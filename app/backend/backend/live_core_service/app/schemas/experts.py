...
class ExpertBase(BaseModel):
    ...
    department: Optional[str] = Field(None, max_length=120, description="[DEPRECATED] V7 后 department 列已删除，请使用 department_id + /admin/expert-departments")
    expertise_areas: Optional[str] = Field(None, description="擅长领域，用于展示和关键词搜索，不作为主分类字段")
    ...
    category_id: Optional[uuid.UUID] = Field(None, description="专家主专业分类ID（关联全局 categories 表，用于筛选、搜索、推荐和统计）。若传入 department_id，应与此科室的 category_id 一致；若省略，创建时从 department_id 自动解析")
    department_id: Optional[uuid.UUID] = Field(None, description="标准化科室ID（关联 expert_departments 表，V4 迁移新增；替代已删除的 department 文本字段）。若传入，创建时自动解析 category_id")
...
class ExpertCreate(ExpertBase):
    ...
    is_verified: Optional[bool] = Field(False, description="审核状态: False=待审批, True=已审批")
...
class ExpertUpdate(BaseModel):
    ...
    title: Optional[str] = Field(None, max_length=120, description="职称")
    hospital: Optional[str] = Field(None, max_length=200, description="所属医院")
    department: Optional[str] = Field(None, max_length=120, description="[DEPRECATED] V7 后 departments 列已删除，请使用 department_id")
    expertise_areas: Optional[str] = Field(None, description="擅长领域，用于展示和关键词搜索，不作为主分类字段")
    bio: Optional[str] = Field(None, max_length=1000, description="个人简介")
    avatar_url: Optional[str] = Field(None, max_length=512, description="头像URL")
    category_id: Optional[uuid.UUID] = Field(None, description="专家主专业分类ID（关联全局 categories 表，用于筛选、搜索、推荐和统计）")
    department_id: Optional[uuid.UUID] = Field(None, description="标准化科室ID（关联 expert_departments 表，V4 迁移新增）")
    ...
    is_verified: Optional[bool] = Field(None, description="审核状态: False=待审批, True=已审批")
...
class ExpertItem(ExpertBase):
    ...
    department_name: Optional[str] = Field(None, description="科室名称（推荐字段，替代已废弃的 department）")
...
class FeaturedExpertItem(BaseModel):
    ...
    department: Optional[str] = Field(None, description="[DEPRECATED] V7 后请使用 department_id + /admin/expert-departments 获取科室名")
    department_name: Optional[str] = Field(None, description="科室名称（推荐字段，替代已废弃的 department）")
    department_id: Optional[uuid.UUID] = Field(None, description="标准化科室ID（关联 expert_departments 表）")
    expertise_areas: Optional[str] = None
    avatar_url: Optional[str] = None
    category_id: Optional[uuid.UUID] = None
    category_name: Optional[str] = Field(None, description="分类名称（JOIN 填充）")
    is_active: bool = True
...
class ExpertSelfUpdate(BaseModel):
    """PR 5: 专家自认领更新 Schema（用户在 /experts/me 自管理）"""
    ...
    department_name: Optional[str] = Field(None, max_length=200, description="科室名称（自动匹配到受控词表）")
