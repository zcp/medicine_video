...
class TopicCategoryInDB(TopicCategoryBase):
    """
    数据库中的专题分类 Schema
...
class TopicCategoryResponse(TopicCategoryInDB):
    """
    专题分类响应 Schema
    
    用于 API 响应，返回专题分类的完整信息。
    """
    pass
...
class TopicCategoryWithRooms(BaseModel):
    """
    包含直播间列表的专题分类 Schema
...
class TopicDetailResponse(TopicInDB):
    ...
    """
    categories: List[TopicCategoryWithRooms] = Field(
        default_factory=list,
