...
class DeactivateCleanupService:
    """按 public_id 清私货、解绑认领/成员，并写已注销标记。幂等。"""
    ...
    async def run(self, user_public_id: uuid.UUID) -> Dict[str, Any]:
        ...
        # 品牌电商已移除，品牌成员表不存在，恒为 0（保留响应字段兼容）
        removed_brand_members = 0

        logger.info(
            "deactivate-cleanup 完成 user=%s unbound_experts=%s brand_members=%s counts=%s",
            user_public_id,
            unbound_experts,
            removed_brand_members,
            deleted_counts,
        )
        return {
            ...
            "removed_brand_members": removed_brand_members,
        }
