...
from app.models.live_core import LiveRoom, LiveSession, LiveSessionStatus, SourceType
...
class SrsCallbackService:
    ...
    async def handle_on_publish(self, stream_key: str) -> LiveSession:
        ...
        # --- 新增的幂等性检查逻辑 ---
        # 2. 检查是否已有 'live' 状态的会话（仅 push 来源，external 场次不受推流回调影响）
        existing_live_session = await crud_session.get_live_session_by_room(
            self.db, room_id=room.id, source_type=SourceType.PUSH
        )
