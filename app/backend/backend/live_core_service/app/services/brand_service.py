...
class BrandService:
    ...
    async def delete_brand(
        ...
        """
        # 权限检查（ADMIN 或 SUPERADMIN）
        check_admin_permission(role)
        
        # 硬删除前检查引用关系（数据关联治理 P0-1，D2 定稿：含焦点图引用）
        if hard_delete:
            topic_count, room_count, featured_count = await crud.check_brand_references(db, brand_id)
            if topic_count > 0 or room_count > 0 or featured_count > 0:
                raise InvalidParameterException(
                    f"品牌被引用无法删除，专题引用数：{topic_count}，直播间引用数：{room_count}，焦点图引用数：{featured_count}（请先下线/改配焦点图再删除）",
                    code=2003
                )
    ...
    async def get_room_brands_for_tab(
        ...
        if include_topic_brands:
            ...
            # 查询专题品牌
            _, topic_brands = await crud.get_brand_with_topics(db, topic_id)
