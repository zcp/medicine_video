...
class TopicService:
    ...
    async def delete_topic(
        ...
        try:
            ...
            # 数据关联治理（P0-1）：先清理关联数据，再删主实体
            # 软下线指向该专题的焦点图，避免 target 悬挂引用（无外键多态关联）
            from app.crud.homepage_search import disable_featured_content_by_target
            await disable_featured_content_by_target(self.db, "topic", topic_id)
    ...
    async def update_category(
        ...
        Raises:
            TopicCategoryNotFoundException: 分类不存在
            PermissionDeniedException: 权限不足
        """
        # 提前提取用于日志的变量
        ...
        try:
            ...
            if not category:
                raise TopicCategoryNotFoundException(str(category_id))
        ...
        except (TopicCategoryNotFoundException, PermissionDeniedException):
            raise
        except Exception as e:
            # ✅ 使用局部变量记录日志
            logger.error(
    ...
    async def get_topics_by_room(
        ...
        try:
            ...
            # → 改为：调用公共函数 check_room_visibility
