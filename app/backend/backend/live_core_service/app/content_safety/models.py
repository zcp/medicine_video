"""内容安全数据库模型（V12 统一结构：UUID/enabled/JSONB/约束）"""
...
class ContentSafetyRule(Base):
    ...
    created_by = Column(UUID(as_uuid=True), nullable=True, comment="创建人 public_id")
    created_at = Column(
...
class ContentSafetyLog(Base):
    ...
    target_field = Column(String(50), nullable=False, comment="目标字段")
    user_id = Column(UUID(as_uuid=True), nullable=True, comment="用户 public_id")
