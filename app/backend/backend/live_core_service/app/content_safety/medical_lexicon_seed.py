...
async def seed_medical_lexicon_rules(db: AsyncSession) -> tuple:
    """幂等写入/同步医学合规规则，返回 (新增数量, 更新数量)"""
    inserted = 0
    updated = 0
    for rule_data in MEDICAL_RULES:
        ...
        # 已有规则：对比 _SYNC_FIELDS，变化则更新（支持分档策略迭代）
        changed = False
    ...
    if inserted or updated:
        await db.flush()
        logger.info("医学合规内容安全规则: 新增 %d 条, 更新 %d 条", inserted, updated)
    return inserted, updated


async def run_medical_lexicon_seed(db: AsyncSession) -> int:
    """完整迁移：停用旧规则 → 写入/同步新规则 → 失效缓存；返回变更总数"""
