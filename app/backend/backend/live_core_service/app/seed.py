...
from app.services.expert_service import _match_broad_category, _normalize_department_text
    ...
    for data in CATEGORIES_DATA:
        ...
        if existing:
            ...
            cat = Category(
                ...
                parent_id=None,
                is_active=True,
                standard=data.get("standard", True),
...
# ============================================================
# 种子内自适应科室解析（同步版，供 seed_experts 使用）
# ============================================================
def _resolve_dept_for_seed(db, department_text: str, expert_name: str):
    """同步版的科室分辨率逻辑，用于 seed 脚本。

    Returns:
        (department_id, category_id) 或 (None, fallback_category_id)
    """
    from app.models.content_management import Category

    normalized = _normalize_department_text(department_text)
    if not normalized:
        fallback_cat = db.query(Category).filter_by(name="其他", is_active=True).first()
        return (None, fallback_cat.id if fallback_cat else None)

    # 1) 按 name 精确匹配已有科室
    dept = db.query(ExpertDepartment).filter_by(name=normalized, is_active=True).first()
    if dept:
        return (dept.id, dept.category_id)

    # 2) 按 synonyms 匹配
    all_depts = db.query(ExpertDepartment).filter(ExpertDepartment.is_active == True).all()
    for d in all_depts:
        if d.synonyms and normalized in d.synonyms:
            return (d.id, d.category_id)

    # 3) 启发式匹配
    heuristic = _match_broad_category(normalized)
    if heuristic:
        matched_dept_name, matched_cat_name = heuristic
        dept = db.query(ExpertDepartment).filter_by(name=matched_dept_name, is_active=True).first()
        if dept:
            return (dept.id, dept.category_id)

    # 4) 确定兜底分类
    if heuristic:
        fallback_cat = db.query(Category).filter_by(name=heuristic[1], is_active=True).first()
    else:
        fallback_cat = db.query(Category).filter_by(name="其他", is_active=True).first()

    if not fallback_cat:
        fallback_cat = db.query(Category).filter(Category.is_active == True).first()
    if not fallback_cat:
        print(f"  [!] 没有可用分类，跳过专家: {expert_name}")
        return (None, None)

    # 5) 自动创建科室（同步版，不依赖异步 CRUD）
    ...
    try:
        import uuid as _seed_uuid
        dept_id = _seed_uuid.uuid4()
        new_dept = ExpertDepartment(
            id=dept_id,
            name=new_dept_data.name,
            category_id=new_dept_data.category_id,
            synonyms=new_dept_data.synonyms or [],
            is_verified=new_dept_data.is_verified,
            source=new_dept_data.source,
            is_active=True,
        )
        db.add(new_dept)
        db.flush()
        print(f"  ++ 自动创建科室: {normalized}")
        return (new_dept.id, new_dept.category_id)
    except Exception:
        db.rollback()
        existing = db.query(ExpertDepartment).filter_by(name=normalized).first()
        if existing:
            return (existing.id, existing.category_id)
        return (None, fallback_cat.id)
...
def seed_experts(db):
    ...
    with open(EXPERTS_CSV_PATH, encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            # 自适应匹配科室 + 分类
            dept_text = (row.get("department") or "").strip()
            dept_id, cat_id = _resolve_dept_for_seed(db, dept_text, row["name"])
            if cat_id is None:
                skip += 1
                continue
...
def seed_live_rooms(db, categories: dict):
    ...
    for room_data in LIVE_ROOMS_DATA:
        ...
        # Sync M2M to live_room_categories
        if cat:
