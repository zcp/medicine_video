...
class Settings:
    ...
    # Redis 直连配置（session revoke / 缓存 / 注销标记 共用）
    # 优先使用 REDIS_URL，fallback 到 CELERY_BROKER_URL
    REDIS_URL: str = os.getenv("REDIS_URL", "")

    # 内部服务间调用 Token
    INTERNAL_SERVICE_TOKEN: str = os.getenv("INTERNAL_SERVICE_TOKEN", "")

    # 管理员是否绕过开播门禁（默认 True，对齐 wechat-v1；False 时管理员也受 can_stream 检查）
    ADMIN_STREAM_BYPASS: bool = os.getenv("ADMIN_STREAM_BYPASS", "true").lower() == "true"

    # 用户服务地址（用于用户资料同步等内部调用）
    USER_SERVICE_URL: str = os.getenv("USER_SERVICE_URL", "http://user_service:8002")

    # 内容安全配置
    CONTENT_SAFETY_ENABLED: bool = os.getenv("CONTENT_SAFETY_ENABLED", "true").lower() != "false"
    CONTENT_SAFETY_WARN_ONLY: bool = os.getenv("CONTENT_SAFETY_WARN_ONLY", "false").lower() == "true"
    POLITICAL_TEXT_API_URL: str = os.getenv("POLITICAL_TEXT_API_URL", "")
    POLITICAL_TEXT_API_KEY: str = os.getenv("POLITICAL_TEXT_API_KEY", "")
    ...
    # JWT 时钟偏差容忍秒数（对齐 users 服务，避免过期前窗口内跨服务行为不对称）
    ...
    # 外部流代理配置（V15）：m3u8 代理端点向外部平台请求时注入的请求头
    # JSON 字符串，如 {"X-API-Key":"xxx","User-Agent":"Mozilla/5.0"}；敏感信息走 .env 管理
    EXTERNAL_STREAM_HEADERS: str = os.getenv("EXTERNAL_STREAM_HEADERS", "")
