...
-- 执行顺序：Step A1（第一步，无依赖）
-- 幂等性：所有 DDL 使用 IF NOT EXISTS / IF EXISTS
...
-- 3) 插入种子科室数据（基于 P3 DEPARTMENT_CATEGORY_MAP 的 41 条数据）
-- 前置条件：categories 表中已存在对应的分类
-- 注意：V4 执行时 parent_id 列尚不存在（V5 才创建），种子查询只用 name + is_active 过滤
DO $$
DECLARE
    cat_id UUID;
BEGIN
    ...
    IF cat_id IS NOT NULL THEN
        INSERT INTO expert_departments (id, name, category_id, is_verified) VALUES
            (gen_random_uuid(), '乳腺外科', cat_id, true),
            (gen_random_uuid(), '甲状腺外科', cat_id, true),
            (gen_random_uuid(), '肝胆外科', cat_id, true),
            (gen_random_uuid(), '胃肠外科一科', cat_id, true),
            (gen_random_uuid(), '胃肠外科二科', cat_id, true),
            (gen_random_uuid(), '胃肠外科三科', cat_id, true),
            (gen_random_uuid(), '胆胰外科', cat_id, true),
            (gen_random_uuid(), '肝外科', cat_id, true)
    ...
    IF cat_id IS NOT NULL THEN
        INSERT INTO expert_departments (id, name, category_id, is_verified) VALUES
            (gen_random_uuid(), '脊柱外科', cat_id, true),
            (gen_random_uuid(), '关节外科', cat_id, true),
            (gen_random_uuid(), '骨肿瘤科', cat_id, true),
            (gen_random_uuid(), '运动医学科', cat_id, true),
            (gen_random_uuid(), '显微创伤外手科', cat_id, true)
        ON CONFLICT (name) DO NOTHING;
    ...
    IF cat_id IS NOT NULL THEN
        INSERT INTO expert_departments (id, name, category_id, is_verified) VALUES
            (gen_random_uuid(), '妇科', cat_id, true),
            (gen_random_uuid(), '产科', cat_id, true)
        ON CONFLICT (name) DO NOTHING;
    ...
    IF cat_id IS NOT NULL THEN
        INSERT INTO expert_departments (id, name, category_id, is_verified) VALUES
            (gen_random_uuid(), '鼻专科', cat_id, true),
            (gen_random_uuid(), '耳专科', cat_id, true),
            (gen_random_uuid(), '咽喉专科', cat_id, true)
        ON CONFLICT (name) DO NOTHING;
    ...
    IF cat_id IS NOT NULL THEN
        INSERT INTO expert_departments (id, name, category_id, is_verified) VALUES
            (gen_random_uuid(), '肾移植专科', cat_id, true),
            (gen_random_uuid(), '器官移植科/肾移植专科', cat_id, true),
            (gen_random_uuid(), '器官移植科/肝移植专科', cat_id, true)
        ON CONFLICT (name) DO NOTHING;
    ...
    IF cat_id IS NOT NULL THEN
        INSERT INTO expert_departments (id, name, category_id, is_verified) VALUES
            (gen_random_uuid(), '泌尿外科/男科', cat_id, true)
        ON CONFLICT (name) DO NOTHING;
    ...
    -- 生殖医学科类（3 条）
    SELECT id INTO cat_id FROM categories WHERE name = '生殖医学科' AND is_active = true LIMIT 1;
    IF cat_id IS NOT NULL THEN
        INSERT INTO expert_departments (id, name, category_id, is_verified) VALUES
            (gen_random_uuid(), '生殖医学中心', cat_id, true),
            (gen_random_uuid(), '生殖男科专科', cat_id, true),
            (gen_random_uuid(), '男科/生殖医学中心', cat_id, true)
        ON CONFLICT (name) DO NOTHING;
        RAISE NOTICE '生殖医学科类种子数据: 3 条';
    ELSE
        RAISE WARNING '分类"生殖医学科"不存在，跳过对应种子数据';
    END IF;
    ...
    IF cat_id IS NOT NULL THEN
        INSERT INTO expert_departments (id, name, category_id, is_verified) VALUES
            (gen_random_uuid(), '胸外科', cat_id, true)
        ON CONFLICT (name) DO NOTHING;
    ...
    IF cat_id IS NOT NULL THEN
        INSERT INTO expert_departments (id, name, category_id, is_verified) VALUES
            (gen_random_uuid(), '口内修复科', cat_id, true),
            (gen_random_uuid(), '口腔颌面外科', cat_id, true)
        ON CONFLICT (name) DO NOTHING;
    ...
    IF cat_id IS NOT NULL THEN
        INSERT INTO expert_departments (id, name, category_id, is_verified) VALUES
            (gen_random_uuid(), '小儿外科', cat_id, true)
        ON CONFLICT (name) DO NOTHING;
    ...
    IF cat_id IS NOT NULL THEN
        INSERT INTO expert_departments (id, name, category_id, is_verified) VALUES
            (gen_random_uuid(), '变态反应专科', cat_id, true),
            (gen_random_uuid(), '外科门诊', cat_id, true),
            (gen_random_uuid(), '肝外科/超声医学科/介入超声专科', cat_id, true)
        ON CONFLICT (name) DO NOTHING;
