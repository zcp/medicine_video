...
async def async_client() -> AsyncGenerator[AsyncClient, None]:
    ...
    from app.api.v1.endpoints.auth import auth_router as auth_router
    from app.api.v1.endpoints.users import users_router as users_router
    from app.api.v1.endpoints.membership_products import membership_products_router as membership_products_router
    ...
    # 添加普通用户路由（prefix 与真实 api.py 对齐）
    app.include_router(auth_router, prefix="/api/v1/auth")
    app.include_router(users_router, prefix="/api/v1/users")
    app.include_router(membership_products_router, prefix="/api/v1/membership-products")
    ...
    try:
        ...
        app.include_router(subscriptions_router, prefix="/api/v1/users/me/memberships")
    except ImportError:
        pass  # 如果路由不存在则跳过
...
def mock_redis():
    ...
    with patch('app.core.redis_client.get_redis_client') as mock_get_redis, \
         patch('app.services.auth_service.get_redis_client') as mock_auth_get_redis:
        # 创建一个模拟的Redis客户端
        ...
        mock_auth_get_redis.return_value = mock_redis_client
