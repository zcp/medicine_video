...
async def get_users_list(
    ...
    keyword: Optional[str] = Query(None, description="统一关键词模糊搜索（跨用户名/邮箱/昵称/手机号）"),
    username: Optional[str] = Query(None, description="按用户名模糊搜索"),
    email: Optional[str] = Query(None, description="按邮箱精确搜索"),
    phone_number: Optional[str] = Query(None, description="按手机号精确筛选"),
    nickname: Optional[str] = Query(None, description="按昵称模糊筛选"),
    role: Optional[UserRole] = Query(None, description="按角色筛选"),
    status: Optional[str] = Query(None, description="按状态筛选"),
    can_stream: Optional[bool] = Query(None, description="按开播资格筛选；false=被禁止开播用户"),
...
async def update_user(
    ...
    try:
        ...
        logger.warning(
            f"管理员更新操作非法: admin_user_id={admin_user_id_for_logging}, "
