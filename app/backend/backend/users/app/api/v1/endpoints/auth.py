...
from app.schemas.auth import SSOLoginRequest, OneTapLoginRequest, CloudFunctionLoginRequest
...
async def get_captcha(db: AsyncSession = Depends(get_async_db)):
    ...
    try:
        ...
        return JSONResponse(
            status_code=500,
            content=error_response(code=1001, message="系统内部错误", data={"error": str(e)})
        )


@auth_router.get("/captcha/image/{captcha_id}")
...
@auth_router.post("/verification-codes")
...
@auth_router.post("/verification-codes/verify")
...
async def login_by_phone(
    ...
    try:
        ...
        return JSONResponse(
            status_code=401,
            content=error_response(code=3001, message="登录凭证无效")
        )
    except Exception as e:
...
async def password_reset(
    ...
    try:
        ...
        logger.warning(f"密码重置验证失败: {e.message}")
        return JSONResponse(
            status_code=400,
            content=error_response(code=4007, message=e.message)
        )
    except Exception as e:
...
@auth_router.post("/one-tap-login")
...
@auth_router.post("/one-tap-login/cloud-function")
