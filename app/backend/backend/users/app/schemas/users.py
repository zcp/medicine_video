...
class UserBase(BaseModel):
    ...
    can_stream: bool = Field(True, description="是否拥有开播资格；默认允许，false 表示被管理员禁播")

    @field_validator("phone_number", mode="after")
    @classmethod
    def _validate_phone_number(cls, v):
        """手机号统一校验并归一化为纯 11 位；None 跳过"""
        if v is None:
            return None
...
class UserUpdate(BaseModel):
    ...
    can_stream: Optional[bool] = Field(None, description="是否拥有开播资格；默认允许，false 表示被管理员禁播")
    ...
    @classmethod
    def _validate_phone_number(cls, v):
        """admin 更新用户资料时手机号统一校验并归一化；None 跳过"""
        if v is None:
            return None
        return validate_cn_phone(v)
...
class UserFilterParams(BaseModel):
    """用户查询过滤参数 Schema"""
    keyword: Optional[str] = Field(None, description="统一关键词模糊搜索（跨 username/email/nickname/phone_number，OR 语义）")
    ...
    can_stream: Optional[bool] = Field(None, description="开播资格筛选；默认允许，false 表示被管理员禁播")
...
class PhoneBindRequest(BaseModel):
    ...
    @classmethod
    def _validate_phone_number(cls, v):
        """校验并归一化手机号，保证与 ticket recipient 比对一致"""
        return validate_cn_phone(v)
