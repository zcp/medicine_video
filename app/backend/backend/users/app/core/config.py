...
class Settings:
    ...
    # Redis URL 直连（session revoke / 跨服务共享）
    REDIS_URL: str = os.getenv("REDIS_URL", "")

    # 内部服务间调用 Token
    INTERNAL_SERVICE_TOKEN: str = os.getenv("INTERNAL_SERVICE_TOKEN", "")

    # LiveCore 服务地址（注销清理等内部调用）
    LIVE_CORE_SERVICE_URL: str = os.getenv("LIVE_CORE_SERVICE_URL", "http://live_core_service:8000")
    ...
    def __init__(self):
        ...
        self.JWT_LEEWAY_SECONDS = int(os.getenv("JWT_LEEWAY_SECONDS", str(self.JWT_LEEWAY_SECONDS)))
    ...
    def _validate_production_config(self):
        ...
        if self.SMS_PROVIDER not in ("mock", "aliyun"):
            errors.append("❌ SMS_PROVIDER 必须是 mock 或 aliyun")
        if self.CARRIER_AUTH_PROVIDER and self.CARRIER_AUTH_PROVIDER not in ("mock",):
            errors.append("❌ CARRIER_AUTH_PROVIDER 只能是 mock 或不设置")
    ...
    def _validate_development_config(self):
        ...
        if self.SMS_PROVIDER not in ("mock", "aliyun"):
            warnings_list.append("⚠️  SMS_PROVIDER 必须是 mock 或 aliyun")
        if self.CARRIER_AUTH_PROVIDER and self.CARRIER_AUTH_PROVIDER not in ("mock",):
            warnings_list.append("⚠️  CARRIER_AUTH_PROVIDER 只能是 mock 或不设置")
