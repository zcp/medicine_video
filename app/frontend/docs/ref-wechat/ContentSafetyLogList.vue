<!--
 * ContentSafetyLogList - 鍙戝竷澶勭悊璁板綍锛堝彧璇伙級
 -->
<template>
  <view class="log-list-page">
    <view class="page-header">
      <view class="header-title">
        <text class="title">鍙戝竷澶勭悊璁板綍</text>
      </view>
    ...
    <view v-if="!loading" class="log-list">
      ...
      <view v-for="item in tableData" :key="item.id" class="log-item">
        ...
        <view class="log-row">
          <text class="log-label">鐢ㄦ埛</text>
          <text class="log-value">{{ displayUserLabel(item) }}</text>
        </view>
        <view class="log-row">
          <text class="log-label">浣嶇疆</text>
          <text class="log-value">{{ formatSceneLabel(item.scene) }}</text>
        </view>
        <view class="log-row">
          <text class="log-label">鐢ㄦ埛濉啓</text>
          <text class="log-value">{{ item.input_excerpt || '鈥? }}</text>
        </view>
      </view>
    </view>
...
function displayUserLabel(item: ContentSafetyLog): string {
  ...
  if (fromPayload !== '鈥? && fromPayload !== item.user_id?.trim()) {
    return fromPayload
  }
...
function needsUserEnrichment(item: ContentSafetyLog): boolean {
  ...
  // 宸叉湁鍙鏄电О/鐢ㄦ埛鍚嶅垯涓嶅啀琛ュ叏
  return label === uid || label === '鈥?
}

/**
 * 鐢ㄧ鐞嗙鐢ㄦ埛鍒楄〃鎸?public_id 琛ュ叏鏄电О锛堟棤鍗曠敤鎴?GET锛涙渶澶氭壂 5 椤碉級
 */
...
async function fetchData() {
  ...
  // 鍒楄〃鍏堝睍绀猴紱鏄电О琛ュ叏寮傛杩涜锛屽け璐ヤ粛鏄剧ず user_id
