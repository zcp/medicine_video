...
**Endpoint**: `DELETE /api/v1/users/me/subscriptions/{subscription_id}`
...
1. 验证JWT Token并提取 `user_id`
2. 查询 `user_subscriptions` 表：`WHERE id = :subscription_id AND user_id = :user_id AND is_active = true`
3. 若不存在或user_id不匹配，返回 `2001` 或 `3002`
...
6. **创建新记录**：
   - 应用层生成UUID
   - 设置`is_latest=true`
   - 填充`progress`、`extra`、`watched_at`字段
