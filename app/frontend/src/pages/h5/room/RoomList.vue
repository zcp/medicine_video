    ...
    <!-- 1. 信息提示条 -->
      ...
      <el-card>
        <el-row justify="space-between" align="middle">
          <el-col>
            <el-space :size="8">
              <el-button type="primary" @click="openCreateModal">创建直播</el-button>
            ...
            <template #default="{ row }">
              <el-space :size="0" :spacer="spacer">
                ...
                <el-dropdown>
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item @click="openEditModal(row)">编辑</el-dropdown-item>
...
const openEditModal = async (room: Room) => {
  ...
  Object.assign(formModel, { id: room.id, title: room.title, description: room.description || '' });
  Object.assign(originalRoomData, { title: room.title, description: room.description || '' });
...
const handleConfirm = async () => {
  ...
  try {
    if (isEditMode.value && formModel.id) {
      const payload: Partial<Room> = {};
      if (formModel.title.trim() !== originalRoomData.title) payload.title = formModel.title.trim();
      if (formModel.description.trim() !== originalRoomData.description) payload.description = formModel.description.trim();
